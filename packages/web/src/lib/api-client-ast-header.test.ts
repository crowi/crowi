...
describe('apiFetch never sends X-Crowi-Ast-Version (RFC-0023 §9)', () => {
  ...
  it('a renderedAst-consuming call (getPage / preview) carries no AST version header', async () => {
    ...
    try {
      await apiClient.pages.$get({ query: { path: '/x' } });
      await apiClient.pages.preview.$post({ json: { body: '# x' } });
    } finally {
      vi.unstubAllGlobals();
    }
