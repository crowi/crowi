 ...
 *
 * The endpoint is the api's `/api/mcp` route, but an MCP client lives
 * outside the browser and cannot resolve a relative URL — unlike
 * `apiBaseUrl()`, which stays relative in the same-origin deployment. So the
 ...
 *      proxy forwards `/api/*` to the api (dev proxy and the distributed
 *      image alike).
...
export const MCP_ENDPOINT_PLACEHOLDER = 'https://<crowi-host>/api/mcp';

/** Absolute `<origin>/api/mcp` URL, or the placeholder when unresolvable. */
export function resolveMcpEndpoint(): string {
  const origin = apiOrigin() || (typeof window !== 'undefined' ? window.location.origin : '');
  return origin ? `${origin}/api/mcp` : MCP_ENDPOINT_PLACEHOLDER;
}
