...
/** Mouse-hover trigger — CodeMirror's built-in `hoverTooltip` owns debounce/positioning/hide-on-mouse-leave (AC-C1's "hover" half). */
function imageHoverTooltip(): Extension {
  return hoverTooltip((view, pos) => computeTooltip(view.state, pos), { hoverTime: 300 });
}
...
const cursorTooltipField = StateField.define<CursorTooltipState>({
  ...
  update(value, tr) {
    ...
    if (value.tooltip && next && value.tooltip.pos === next.pos && value.tooltip.end === next.end) {
      return value;
    }
    return { tooltip: next };
  },
...
export function imageAffordanceExtension(): Extension {
  return [cursorTooltipField, imageHoverTooltip(), affordanceTheme];
}
