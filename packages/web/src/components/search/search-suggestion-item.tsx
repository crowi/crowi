...
import { Compass, LayoutFreeform, Lock } from 'lucide-react';
...
export function SearchSuggestionItem({ href, page, snippet, onClick }: SearchSuggestionItemProps) {
  ...
  // RFC-0020 — same rule as `PageListItem`: an icon, not a pill, read
  // from the denormalized `Page.contentType` hint.
  ...
  return (
    <Link href={href} onClick={onClick} className="block rounded-md px-3 py-2 text-sm hover:bg-accent transition-colors">
      <div className="flex items-center gap-2 min-w-0">
        ...
        {isArtifact && <LayoutFreeform className="h-3.5 w-3.5 text-muted-foreground shrink-0" aria-label="HTML artifact page" />}
      </div>
      {snippet && (
