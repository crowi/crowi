 ...
 *
 * Built on the shared `Sheet` (Radix `Dialog`) primitive rather than a
 * hand-rolled `createPortal` overlay — focus trap, focus restore to the
 * trigger, Esc-to-close, `aria-modal` and background scroll lock all come
 * from Radix instead of being reimplemented here. `side="top"` is pushed
 * to full-screen via `className` (`cn()`'s tailwind-merge lets these
 * later utilities win over `sideClasses.top`'s `h-auto`/`border-b`
 * defaults). The corner ✕ close button `SheetContent` renders by default
 * is hidden — the existing back-arrow button already covers that role and
 * a second close affordance floating over the search bar would be
 * redundant on a full-screen surface.
 */
export function MobileSearch() {
  ...
  // unmounted and `debouncedQ` is reset (see `handleOpenChange`), so
  // neither the search nor the recents query does any background work.
  useEffect(() => {
    if (!open) return;
  ...
  // Every close path (Esc, overlay click, the back arrow, a suggestion
  // pick, submit) routes through Radix's `onOpenChange`, so the query
  // reset lives in exactly one place instead of being duplicated per
  // close affordance.
  ...
  // Named so every close affordance below (back arrow, suggestion pick,
  // submit) reads the same as before the Sheet migration.
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent
        ...
        onOpenAutoFocus={(event) => {
          // Radix's default `onOpenAutoFocus` moves focus to the content
          // panel itself; preserve the existing "search bar auto-focuses"
          // UX by focusing the input instead.
          event.preventDefault();
          inputRef.current?.focus();
        }}
        className="md:hidden inset-0 h-screen w-full max-w-none flex-col gap-0 rounded-none border-0 p-0 [&>[data-slot=sheet-close]]:hidden"
      >
        {/* Visually hidden — the search bar itself communicates purpose;
            Radix requires a Dialog.Title for assistive tech. */}
        <SheetTitle className="sr-only">{m['search.global.placeholder']()}</SheetTitle>
