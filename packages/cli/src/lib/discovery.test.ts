...
function discoveryDoc(overrides: Partial<Record<string, string>> = {}): Record<string, unknown> {
  return {
    ...
    device_authorization_endpoint: 'https://wiki.example.com/api/v2/oauth/device',
...
describe('discover — origin validation (OAuth metadata mix-up defense)', () => {
  it('accepts a same-origin https discovery document without warning', async () => {
    ...
    expect(endpoints.deviceEndpoint).toBe('https://wiki.example.com/api/v2/oauth/device');
  ...
  it('rejects a token_endpoint on a foreign origin', async () => {
    fetchMock.mockResolvedValue(jsonResponse(200, discoveryDoc({ token_endpoint: 'https://evil.example.net/api/v2/oauth/token' })));
  ...
  it('rejects a revocation_endpoint on a foreign origin', async () => {
    fetchMock.mockResolvedValue(jsonResponse(200, discoveryDoc({ revocation_endpoint: 'https://evil.example.net/api/v2/oauth/revoke' })));
    await expect(discover('https://wiki.example.com')).rejects.toThrow(/issuer origin/);
  });

  it('rejects a device_authorization_endpoint on a foreign origin', async () => {
    fetchMock.mockResolvedValue(jsonResponse(200, discoveryDoc({ device_authorization_endpoint: 'https://evil.example.net/api/v2/oauth/device' })));
    await expect(discover('https://wiki.example.com')).rejects.toThrow(/issuer origin/);
  });
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse(
        ...
        discoveryDoc({
          ...
          device_authorization_endpoint: 'http://localhost:4301/api/v2/oauth/device',
        }),
      ),
    );
    const endpoints = await discover('http://localhost:4301');
    expect(endpoints.tokenEndpoint).toBe('http://localhost:4301/api/v2/oauth/token');
    expect(stderr).not.toHaveBeenCalled();
  });
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse(
        ...
        discoveryDoc({
          ...
          token_endpoint: 'http://wiki.example.com/api/v2/oauth/token',
          revocation_endpoint: 'http://wiki.example.com/api/v2/oauth/revoke',
          device_authorization_endpoint: 'http://wiki.example.com/api/v2/oauth/device',
        }),
      ),
    );
    const endpoints = await discover('http://wiki.example.com');
    expect(endpoints.tokenEndpoint).toBe('http://wiki.example.com/api/v2/oauth/token');
