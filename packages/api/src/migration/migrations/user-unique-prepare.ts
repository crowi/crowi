...
export const userUniquePrepare = defineMigration({
  ...
  toVersion: '2.0',
  layer: 'preflight',
  // Blocking: dedups so the unique index can build without E11000 (RFC
  // §9/§11). Booting unapplied risks an autoIndex failure, so a pending
  // verdict must refuse boot under the `block` policy. Do NOT reclassify to
  // cosmetic — that would re-expose E11000.
  severity: 'blocking',
