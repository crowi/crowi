...
describe('Routes /api/pages/:id/presence-token (Hono getPresenceToken)', () => {
  ...
  it('returns 401 when no Authorization header is provided', async () => {
    const res = await request(app).get('/api/pages/000000000000000000000000/presence-token').set('Content-Type', 'application/json');
  ...
  it('returns 400 INVALID_PAGE_ID when :id is not a 24-char hex string', async () => {
    const res = await request(app).get('/api/pages/not-a-valid-id/presence-token').set(authHeaders(accessToken));
  ...
  it('returns 404 PAGE_NOT_FOUND for a well-formed but nonexistent page id', async () => {
    const res = await request(app).get('/api/pages/000000000000000000000000/presence-token').set(authHeaders(accessToken));
  ...
  it('returns 404 PAGE_NOT_FOUND when the caller lacks read permission (does not leak existence)', async () => {
    ...
    const res = await request(app).get(`/api/pages/${pageId}/presence-token`).set(authHeaders(otherAccessToken));
  ...
  it('returns 200 with a presence token, mirroring pageId / selfUserId / ISO expiresAt', async () => {
    ...
    const res = await request(app).get(`/api/pages/${pageId}/presence-token`).set(authHeaders(accessToken));
