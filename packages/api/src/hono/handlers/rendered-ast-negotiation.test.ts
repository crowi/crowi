...
describe('renderedAst content negotiation (RFC-0023 §9)', () => {
  ...
  beforeAll(async () => {
    ...
    pageId = page._id;
    const res = await request(app).get('/api/pages').query({ page_id: pageId }).set(authHeaders(accessToken));
  ...
  describe('GET /pages (getPage)', () => {
    ...
    it('X-Crowi-Ast-Version: 1 → envelope with the shiki fence projected to a tokens-carrying code node', async () => {
      const res = await request(app).get('/api/pages').query({ page_id: pageId }).set(authHeaders(accessToken)).set(AST_HEADER, '1');
      expect(res.status).toBe(200);
    ...
    it('an unsupported declared version behaves like no declaration', async () => {
      const res = await request(app).get('/api/pages').query({ page_id: pageId }).set(authHeaders(accessToken)).set(AST_HEADER, '99');
      expect(res.status).toBe(200);
  ...
  describe('GET /pages/list (listPages — portal document path)', () => {
    it('sets Vary on the list response (the portal document is the only renderedAst carrier)', async () => {
      const res = await request(app).get('/api/pages/list').query({ path: '/ast-nego/' }).set(authHeaders(accessToken)).set(AST_HEADER, '1');
      expect(res.status).toBe(200);
      expect(res.headers.vary).toContain(AST_HEADER);
    });
  });
  ...
  describe('GET /pages/revisions/:id (getRevision)', () => {
    it('declaration-less → bare Root + Vary; v1 → envelope + artifact key', async () => {
      const legacy = await request(app).get(`/api/pages/revisions/${revisionId}`).set(authHeaders(accessToken));
      expect(legacy.status).toBe(200);
      expect(legacy.body.revision.renderedAst.type).toBe('root');
      expect(legacy.headers.vary).toContain(AST_HEADER);

      const v1 = await request(app).get(`/api/pages/revisions/${revisionId}`).set(authHeaders(accessToken)).set(AST_HEADER, '1');
  ...
  describe('POST /pages/preview', () => {
    it('goes through the same chokepoint: v1 → envelope with data-source-line hProperties preserved; artifact key is a per-response nonce', async () => {
      const res = await request(app).post('/api/pages/preview').set(authHeaders(accessToken)).set(AST_HEADER, '1').send({ body: '# Title\n\ntext line' });
      expect(res.status).toBe(200);
      ...
      const res2 = await request(app).post('/api/pages/preview').set(authHeaders(accessToken)).set(AST_HEADER, '1').send({ body: '# Title\n\ntext line' });
    ...
    it('declaration-less preview keeps the bare Root shape', async () => {
      const res = await request(app).post('/api/pages/preview').set(authHeaders(accessToken)).send({ body: '# Title' });
      expect(res.status).toBe(200);
