 ...
 * `packages/api/src/hono/handlers/app.ts`). The wire format must stay
 * identical to the ts-rest era: `{ title: string | null }` where
 * `title` is `null` when the operator has not customised it (the
 * `'Crowi'` seed value counts as "not customised").
 */
describe('GET /api/v2/app/info (Hono)', () => {
  let Config: ReturnType<typeof crowi.model<'Config'>>;
  const APP_KEYS = ['app:title'];
  ...
  it('returns the configured title when the operator has customised it', async () => {
    ...
    const res = await request(app).get('/api/v2/app/info');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ title: 'My Wiki' });
  });
});
