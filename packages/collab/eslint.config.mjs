...
// guard as `packages/api/eslint.config.mjs` — block a test file from
// opening its own ad hoc DB connection instead of going through the harness
// (`__tests__/setup.ts`'s `startInMemoryMongo()`).
const dbBypassRules = {
...
export default defineConfig([
  // eslint.config.mjs's flat resolution walks up from cwd (findUp), so a
  // package without its own config file would otherwise inherit the repo
  // root one — `@crowi/collab` needs its own DB-bypass guard below, so it
  // needs its own file regardless. Flat config has no `root: true`
  // ancestor-stop; the nearest config file found IS the final config, no
  // merging with anything above it.
  globalIgnores([
    'dist/**',
    'coverage/**',
    'tmp/**',
  ...
  ]),
  {
    ...
    // equivalent — classic config only reports these with the
    // `--report-unused-disable-directives` CLI flag, which `pnpm lint`
    // never passed. Left at the flat default, every `// eslint-disable-
    // next-line @typescript-eslint/no-explicit-any` in a test file (legal
    // under the override below, which turns that rule off there) newly
    // warns as "unused". Not a rule content change, but the same class of
    // migration-introduced noise AC-9 asks not to add.
    linterOptions: {
