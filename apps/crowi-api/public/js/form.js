(self["webpackChunkcrowi"] = self["webpackChunkcrowi"] || []).push([["form"],{

/***/ "./client/crowi-form.ts":
/*!******************************!*\
  !*** ./client/crowi-form.ts ***!
  \******************************/
...
$(function () {
  var crowi = window.crowi;
  var crowiRenderer = window.crowiRenderer;
  var inlineAttachment = window.inlineAttachment;
  var pageId = $('#content-main').data('page-id');
  var pagePath = $('#content-main').data('path'); // show/hide

  function FetchPagesUpdatePostAndInsert(path) {
    $.get('/_api/pages.updatePost', {
      path: path
    }, function (res) {
      if (res.ok) {
  ...
  var slackConfigured = $('#page-form-setting').data('slack-configured'); // for new page
  ...
  $('a[data-toggle="tab"][href="#edit-form"]').on('show.bs.tab', function () {
    ...
    if (slackConfigured) {
      var $slackChannels = $('#page-form-slack-channel');
      var slackChannels = $slackChannels.val(); // if slackChannels is empty, then fetch default (admin setting)
  ...
  var originalContent = $('#form-body').val(); // restore draft
  ...
  function renderPreview() {
    ...
    var previewBodyElement = previewBody[0];
  ...
  var insertText = function insertText(start, end, newText, mode) {
  ...
  var getCurrentLine = function getCurrentLine(event) {
  ...
  var handleTabKey = function handleTabKey(event) {
    ...
    var currentLine = getCurrentLine(event);
    var text = $target.val();
    var pos = $target.selection('getPos'); // When the user presses CTRL + TAB, it is a case to control the tab of the browser
    ...
    if (event.shiftKey) {
      ...
      if (currentLine && currentLine.text.charAt(0) === '|') {
        // next cell in table
        var _newPos = text.indexOf('|', pos.start + 1);

        if (_newPos < 0 || _newPos === text.lastIndexOf('|', currentLine.end - 1)) {
          $target.selection('setPos', {
        ...
        // indent
        var isBlankLine = !(currentLine === null || currentLine === void 0 ? void 0 : currentLine.text);
        $target.selection('replace', {
  ...
  var handleEnterKey = function handleEnterKey(event) {
    ...
    if (match) {
      ...
      if (!prevLine || !currentLine.text.match(/---/) && !prevLine.text.match(/\|/g)) {
        ...
        var _pos = $target.selection('getPos');

        insertText(_pos.start, _pos.start, '\n' + row.join(' --- ') + '\n' + row.join('  '), 'after');
        $target.selection('setPos', {
        ...
        // $target.selection('insert', {text: "\n" + row.join('  '), mode: 'before'});
        var _pos2 = $target.selection('getPos');

        insertText(_pos2.start, _pos2.end, '\n' + row.join('  '), 'after');
        $target.selection('setPos', {
  ...
  var handleSpaceKey = function handleSpaceKey(event) {
  ...
  var unbindInlineAttachment = function unbindInlineAttachment($form) {
    $form.unbind('.inlineattach');
  };

  var bindInlineAttachment = function bindInlineAttachment($form, attachmentOption) {
    ...
    $form.bind({
      'paste.inlineattach': function pasteInlineattach(e) {
        inlineattach.onPaste(e.originalEvent);
      },
      'drop.inlineattach': function dropInlineattach(e) {
        e.stopPropagation();
        e.preventDefault();
        inlineattach.onDrop(e.originalEvent);
      },
      'dragenter.inlineattach dragover.inlineattach': function dragenterInlineattachDragoverInlineattach(e) {
        e.stopPropagation();
        e.preventDefault();
      }
    });
  ...
  var createEditorInstance = function createEditorInstance($form) {
    var $this = $form;
    return {
  ...
  if ($inputForm.length > 0) {
    ...
    var _pageId = $('#content-main').data('page-id') || 0;

    var _pagePath = $('#content-main').data('path');
    ...
    var _crowi$getContext = crowi.getContext(),
        upload = _crowi$getContext.upload;
    ...
    attachmentOption.onFileReceived = function (file) {
      ...
      if (!file.type.match(/^image\/.+$/)) {
        ...
        this.settings.urlText = "[".concat(file.name, "]({filename})\n");
      } else {
        this.settings.urlText = "![".concat(file.name, "]({filename})\n");
      }
    };
    ...
    attachmentOption.onFileUploadResponse = function (res) {
      ...
      if (result.ok && result.pageCreated) {
        ...
        attachmentOption.extraParams.page_id = _pageId2;
  ...
  var enableScrollSync = function enableScrollSync() {
    var getMaxScrollTop = function getMaxScrollTop(dom) {
      var rect = dom.getBoundingClientRect();
      return dom.scrollHeight - rect.height;
    };

    var getScrollRate = function getScrollRate(dom) {
      var maxScrollTop = getMaxScrollTop(dom);
      var rate = dom.scrollTop / maxScrollTop;
      return rate;
    };

    var getScrollTop = function getScrollTop(dom, rate) {
      var maxScrollTop = getMaxScrollTop(dom);
...
/******/ var __webpack_exports__ = (__webpack_exec__("./client/crowi-form.ts"));
/******/ }
]);
