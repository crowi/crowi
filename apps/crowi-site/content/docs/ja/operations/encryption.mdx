...
登録されたキーだけです。Crowi にあらかじめ組み込まれている対象は次の
とおりです。
...
| `crowi:upload:aws:accessKeyId` / `secretAccessKey` | アップロード用 AWS クレデンシャル |
| `crowi:mail:aws:accessKeyId` / `secretAccessKey` | メール (SES) 用 AWS クレデンシャル |
| `crowi:mail:smtpPassword` | SMTP パスワード |
| `notification:slack:clientSecret` / `token` | Slack 連携のシークレット / トークン |
| `crowi:plugin:@crowi/plugin-aws:accessKeyId` / `secretAccessKey` | AWS プラグインのクレデンシャル |

これに加えて、プラグインが `@sensitive` マーカーを付けた設定フィールドも
起動時に自動で機密リストへ登録され、同じ暗号化経路をたどります
(`crowi:plugin:<プラグイン名>:<フィールド>`)。
