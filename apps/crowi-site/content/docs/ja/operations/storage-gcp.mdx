---
title: GCS ストレージのセットアップ
description: GCS ドライバのために Google Cloud 側でバケット・サービスアカウント・権限を用意する
---

このページは **Google Cloud 側の準備手順** だけを扱います。用意したバケットを Crowi に登録する手順 (管理画面のフィールド、移行手順) は [ストレージ設定](./storage) を参照してください。
...
バケットとクレデンシャルが用意できたら、[ストレージ設定](./storage) の GCS の節に進んでください。
