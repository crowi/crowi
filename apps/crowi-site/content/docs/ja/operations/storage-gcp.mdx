...
このページは **Google Cloud 側の準備手順** だけを扱います。用意したバケットを Crowi に登録する手順 (管理画面のフィールド、移行手順、エミュレータ) は [ストレージドライバ](./storage) を参照してください。
...
バケットとクレデンシャルが用意できたら、[ストレージドライバ](./storage) の GCS の節に進んでください。
