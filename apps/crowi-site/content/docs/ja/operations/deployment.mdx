...
| **WebSocket `wsToken`** | ブラウザ。realtime の 3 経路(`/collab`・`/presence`・`/notifications`)向け——認証済み Bearer 呼び出し(例: `GET /api/pages/{id}/yjs-token`)で発行される短命・単一用途のトークンを、WebSocket 接続時に `?token=` クエリパラメータとして渡す。ブラウザは WS handshake に `Authorization` ヘッダを付けられないため([Realtime collaboration](./realtime-collab) と RFC-0003 参照) | `wss://wiki.example.com/collab?token=…` | **可**(トークンはヘッダではなく URL に乗る) |
...
短命な `wsToken` をクエリパラメータとして渡す方法——で認証します(WS handshake には
カスタムヘッダを付けられないため)。インライン `<img>` 添付とファイルダウンロードだけは、
ブラウザがヘッダを付けられないまま自分で取得するため first-party の `crowi.accessToken`
cookie にフォールバックします。feature-auth-cookie-fallback-scope 以降、この cookie フォールバックは
...
[設定](./configuration) の環境変数表参照)。ただしこれは api プロセス内で判定される
...
**Caddy には既定の body size 制限がありません。** 上記の Caddy 設定例のままで、
crowi の上限一杯までアップロードが通ります。

<Callout type="warn">
  プロキシの上限が crowi の上限より小さいと、CLI の pre-flight チェック(`GET
  /attachments/upload-policy` を見て判定)は通過するのに、実際のアップロードでは
...
## このバージョンへの更新 — 協調的なレプリカ入れ替えが必須

<Callout type="warn">
  **アカウント連携リリースへの更新は、全 api レプリカを揃えて drain し
  入れ替える必要があります。1 台ずつのローリング再起動は行わないで
  ください。** 新バージョンのレプリカは
  アカウント連携フロー用にフロー固有の link-state cookie
  (`crowi.oauthLinkState.<state>`) を発行します。旧バージョンのレプリカの
  callback ハンドラはこの cookie namespace を認識せず、固定のサインイン
  用 state cookie (`crowi.oauthState`) を読もうとし、しかも検証する前に
  それを削除してしまいます。混在した fleet では、たまたま旧バージョンの
  レプリカに着地した callback が進行中のアカウント連携を壊すだけでなく、
  **無関係かつ同時に進行していた通常のサインイン**まで、その固定 state
  cookie を消してしまうことで壊してしまいます。全 api レプリカを揃えて
  停止し、全レプリカへ新バージョンをデプロイしてから揃えて再起動して
  ください — あるいは新バージョンで新しい fleet を用意しておき、
  トラフィックを一括で切り替える (blue/green) 方法でも構いません。
  2 つのバージョンをつなぐ互換 route やフォールバック cookie はありません。
  local dev を含む単一インスタンス構成は自動的にこの条件を満たします —
  レプリカが 1 台しかないため、新旧バージョンの混在はそもそも起こりません。
</Callout>

**同じ入れ替えが、ページ履歴のためにも必要です。** ページの rename・ゴミ箱への移動・復元が「移動中」という新しい状態を経由するようになりました。この状態を知らない旧バージョンのレプリカは、移動中のページを通常のページとして扱い、別の移動を重ねて開始しえます。そうなるとどちらの移動も完了せず、ページは移動中のまま取り残されます。

混在させてしまった場合は、全レプリカを入れ替えたうえで次を実行すると回収できます。
