...
## このバージョンへの更新は全レプリカを同時に入れ替える

複数レプリカで api を動かしている場合、**このバージョンへの更新に限り、ローリング更新ではなく全レプリカを同時に入れ替えてください。** 単一インスタンス運用なら自動的に満たされるので、何もする必要はありません。

理由は、ページの rename・ゴミ箱への移動・復元が「移動中」という新しい状態を経由するようになったためです。この状態を知らない旧バージョンのレプリカは、移動中のページを通常のページとして扱い、別の移動を重ねて開始しえます。そうなるとどちらの移動も完了せず、ページは移動中のまま取り残されます。
