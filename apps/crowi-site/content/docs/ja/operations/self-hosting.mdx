...
フィールドの意味とドライバ名の対応は [crowi.config.json](../reference/config-file) を参照してください。
...
api が起動時に読む環境変数を用意します。`MONGO_URI` と `SECRET_TOKEN` は
必須です(`SECRET_TOKEN` が未設定・空・空白のみ・既知の placeholder の
場合、api は起動しません)。本番では `CROWI_ENCRYPTION_KEY` も必ず設定
してください。
...
SECRET_TOKEN=...                       # openssl rand -base64 32、全レプリカで同じ値
```

各変数の意味は [環境変数](../reference/env) を参照してください。
...
機密値（`CROWI_ENCRYPTION_KEY` / `SECRET_TOKEN` など）は **イメージに
...
<Callout type="info">
  複数レプリカで運用する場合は、`REDIS_URL` と `CROWI_MULTI_INSTANCE` の
  扱いが変わります。`SECRET_TOKEN` は単一・複数レプリカのどちらでも必須
  ですが、複数レプリカでは **全レプリカを完全に同じ値** にする必要が
  あります。値を変更する(ローテーションする)場合は、旧レプリカを
  drain・停止してから新しい値の新版レプリカを起動する
  drain-first の手順を踏んでください — 新旧レプリカが異なる値で同時に
  稼働する期間を作ってはいけません。[環境変数](../reference/env) と
