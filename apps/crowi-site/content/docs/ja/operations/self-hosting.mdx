...
web は `packages/web/Dockerfile` からビルドします。ここで **最重要** なのが
`NEXT_PUBLIC_API_URL` です。

<Callout type="warn">
  `NEXT_PUBLIC_API_URL` は **ビルド時に静的に焼き込まれます**（Next.js の
  `NEXT_PUBLIC_*` の仕様）。実行時の環境変数では変えられません。**ブラウザから
  到達できる API の URL** を、必ず `--build-arg` で渡してビルドしてください。
  コンテナ間のホスト名（`http://api:3000` など）はブラウザからは解決できないため
  不可です。
</Callout>

### 単一ドメイン + リバースプロキシ（推奨）

web と api を同一オリジンで配信し、プロキシで振り分ける構成です。この場合
`NEXT_PUBLIC_API_URL` は **公開オリジンそのもの** を指定します。
...
`NEXT_PUBLIC_COLLAB_URL` は **省略推奨** です。未設定なら web は
`window.location.host` と `NEXT_PUBLIC_API_URL` から WebSocket URL を導出するため、
`/collab/*` を同一ホストにプロキシしていればそのまま動きます。

### api / web を別オリジンで配信する場合

別ドメイン（例: web=`https://wiki.example.com`、api=`https://api.example.com`）で
配信する場合は、`NEXT_PUBLIC_API_URL=https://api.example.com` でビルドし、api 側で
`CLIENT_URL=https://wiki.example.com` を設定して CORS を許可します。`/collab/*` を
api ドメイン側に逃がす場合は `NEXT_PUBLIC_COLLAB_URL` を明示します。
