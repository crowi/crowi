---
title: リリース runbook
description: Crowi 2.0 のリリース手順 — changesets + npm Trusted Publishing(OIDC) + Docker(full/slim) + Elasticsearch カスタム image を CI で公開する
---
...
| Docker full/slim を multi-arch build+push | publish を伴う Release run の完了 | CI（`docker.yml`、`workflow_run`） |
