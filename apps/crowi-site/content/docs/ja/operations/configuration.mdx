...
| `NEXT_PUBLIC_COLLAB_URL` | いいえ | ブラウザが接続する WebSocket URL。未指定時は `NEXT_PUBLIC_API_URL` + `window.location` から導出する。api と web が **別ホスト** のとき (= `/collab/*` を web と同じホストにリバースプロキシしていないとき) のみ設定 |
| `NEXT_PUBLIC_API_URL` | Web で必要 | ブラウザから見た api のオリジン。これは **web 側** の変数で、Docker イメージではビルド時に焼き込まれる (compose のデフォルトは `http://api:3000`) |
