---
title: CLI (コマンドライン)
description: "`@crowi/cli` でターミナルから wiki を検索・読み取り・作成・編集する"
---
...
個人アクセストークンは **設定 → パスワード/APIトークン/アカウント連携** から、
スコープと有効期限を指定して発行します ([アカウントとセキュリティ](./account-security))。
...
[crowi CLI](../reference/cli) のコマンド表を参照してください。
...
crowi search "release notes"   # --profile なしで almoha に対して実行される
```

存在しない alias を指定すると、設定ファイルを変更せずに
...
サブコマンド・固有のフラグ・必要なスコープの一覧は
[crowi CLI](../reference/cli) にあります。引数は送信前に対応する API
リクエストスキーマで検証されるため、不正な path などは往復せずローカルで
明確なエラーになります。

### artifact ページを書く (`--type`)

`create` / `update` は `--type markdown` または `--type artifact` でページの種別を宣言できます。省略するとサーバの既定に従います — `create` は markdown のページになり、`update` は現在の種別をそのまま保ちます。`--type` を省略した update が artifact ページを markdown に戻すことはありません。いずれの場合も種別は `--file` の拡張子や本文の中身から推測されません。`--type artifact` を指定するときの本文は、**単一の自己完結 HTML document** である必要があります — 外部の script / stylesheet / image / module を参照せず、相対パスも使わず、バイナリは `data:` URI で埋め込みます。

`edit` は artifact ページでも markdown ページと同じように使えます。`.md` ではなく `.html` の一時ファイルを開く (エディタのシンタックスハイライトが本文と噛み合う) だけで、保存時にページの種別が変わることはありません。

artifact の書き込みが拒否されると終了コード `6` (invalid) になり、サーバの固定メッセージに規則の識別子と理由が付いて表示されます:
...
crowi attach download 6650f0c1a2b3c4d5e6f70123 > design.pdf   # -o 省略時は stdout
```

`-o` を省略すると stdout に書き出します (端末に直接出力しようとした場合はバイナリで表示が壊れるため拒否します)。進捗や確認メッセージは stderr に出るので、リダイレクトやパイプにバイナリ以外が混ざることはありません。
...
取得した policy はプロファイルに 10 分キャッシュされるので、実行のたびに往復はしません。**この endpoint を持たない古いサーバ**は `404` を返し、CLI は同梱の拡張子テーブルに切り替えます — 事前チェックが無くなるだけで、アップロード自体は従来どおり動きます。

## 出力フォーマットとスクリプティング

`search` と `ls` は `--format` / `--template` で出力を整形でき、パイプ処理に
向きます。指定できる値と終了コードの一覧は
[crowi CLI](../reference/cli) にあります。
...
進捗・警告はすべて stderr に出るため、`--json` の標準出力はスクリプト用に
クリーンなままです。
...
- [アカウントとセキュリティ](./account-security) — 個人アクセストークンの発行と失効
- [AI ツールから使う (MCP)](./mcp) — 同じトークンを AI クライアントから使う
- [crowi CLI](../reference/cli) — コマンド・フラグ・終了コードの一覧
