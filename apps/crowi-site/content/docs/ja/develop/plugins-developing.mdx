...
| `registerNotifier` | 通知プラグイン |
| `registerRenderer` | Markdown レンダラ拡張 |
| `registerRoutes` | プラグイン独自の HTTP エンドポイント。登録したルートは `/api/plugins/<プラグイン名>/<パス>` にマウントされ、`auth: 'user' | 'admin' | 'public'` で認証の要否を選べます (`public` は Slack の署名検証のように、自前で認証する webhook 向け)。 |
