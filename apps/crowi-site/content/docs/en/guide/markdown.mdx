...
code blocks, and so on — works as-is. To look up a syntax on its own, see the
cheat sheet in [Markdown syntax](../reference/markdown).
...
- Each heading shows a small **anchor copy button** on hover. Clicking it
  copies that section's URL to the clipboard and briefly turns into a
...
start with `/`, and external URLs such as `http(s)://`, are shown dimmed as a
"broken wiki link".

### Ordinary links to a page whose path has a space

A Crowi page path can contain spaces, but under CommonMark's rules the
destination (inside the parentheses) of an ordinary Markdown link
`[label](url)` cannot contain a raw space. To link to a page whose path has
a space, use one of the following notations.

| Notation | Example | Notes |
| -------- | ------- | ----- |
| `%20` (recommended) | `[AI report](/survey/AI%20report%20draft)` | Explicitly valid under RFC 3986 and the most interoperable with external Markdown tooling (Pandoc, other CommonMark implementations) |
| `+` | `[Weekly report](/crowi/weekly+report+2026+07)` | Matches Crowi's own URL convention of representing a space as `+`. Still supported |
| `<...>` (angle-bracket) | `[label](</foo bar>)` | The CommonMark-standard angle-bracket destination form — a raw space can be written as-is |

A raw space written directly in the destination, as in `[label](/foo bar)`,
does not become a link under CommonMark's standard interpretation. Crowi,
however, leniently recovers it into a clickable internal link to the actual
`/foo bar` page (**this is an intentional deviation from CommonMark**). Only
an ordinary link with an absolute-path (starting with `/`) destination is
recovered; the following stay literal text as typed:

- Image syntax (`![alt](/foo bar)`)
- An escaped form, e.g. `\[label\](/foo bar)`
- Inside a fenced code block or inline code
- Nested inside an existing link's label (e.g. `[outer [x](/foo bar) part](/dest)`)

If interoperability with other CommonMark-compliant tooling matters, prefer
one of the explicit `%20` / `+` / `<...>` notations above instead of relying
on a raw space.
...
emoji. This is a Crowi core feature that requires no installation and is
always on.
...
The `@[tag](url)` syntax invokes an embed. `card` is a built-in tag reserved
by Crowi core itself; any other `tag` is provided by a renderer plugin. When
no embed renderer is registered for `tag`, it is displayed as-is — `@`
followed by a plain inline link.

#### Link cards `@[card](url)`

Writing `@[card](url)` fetches the target page's OGP meta tags (`og:title` /
`og:description` / `og:image` / `og:site_name`) and renders a link-preview
card. This is a Crowi core feature that requires no installation and is
always on (see "Markdown features built into core" in
[Renderer plugins](../operations/renderers) for details).
...
- A page with no `og:image` renders as a text-only card (title, description,
  domain — no image).
- You can disable this feature entirely from the admin UI under Security →
  "Allow link cards for external URLs" (on by default). Whether the feature is
  disabled, the OGP fetch failed, or the host is unreachable, the card
  degrades to a unified fallback card that shows just the URL as its title —
  no OGP title/description/image, and no error-red styling — while still
  working as a link. If the card rendered successfully before, a temporary
  fetch failure keeps the previous card on screen.
...
- The editor's live preview (before saving) does not fetch real OGP data —
  ...
  you save. This placeholder is deliberately the same visual contract as the
  unified fallback card above.
- The image is a direct link to the source site (no proxying or caching).
  See [Renderer plugins](../operations/renderers) for SSRF safeguard details.
...
> `![filename](/api/attachments/<id>)` with no attributes. Add a
> `{...}` block by hand afterward if you want to set its size or
> placement (see [Attachments](./attachments)).

### GitHub Alerts `> [!NOTE]`

A block quote whose **first line is nothing but a marker** renders as a typed callout. Same notation as GitHub, with five kinds — `NOTE`, `TIP`, `IMPORTANT`, `WARNING`, `CAUTION` (case-insensitive).

```markdown
> [!WARNING]
> Config keys are renamed from this version on.
> The old keys are ignored.
```

- **The marker goes on its own line, first.** `> [!NOTE] body` — content on the same line — is not a callout and stays an ordinary quote, matching GitHub.
- **Escaped forms stay literal.** Neither `\[!NOTE]` nor `&#91;!NOTE]` is read as a marker, which is how you write about the notation itself.
- Only a quote **at the top level of the document** qualifies; one nested inside a list or another quote does not.

### Frontmatter at the top of a page

When a document **opens** with a `---`-delimited block, its contents are lifted out of the body and rendered as a metadata table above it. Pasting a spec, an RFC, or anything else carrying metadata headers no longer mangles the body.
...
- **This is not a YAML parse.** Each line is read as `key: value` and nothing more, so anchors and aliases are never expanded and every value stays a plain string.
- The limits are **50 entries, 100 characters per key, 300 per value, and 8KiB for the whole block**.
- **Anything over those limits, or that cannot be read as entries, is shown verbatim as a `yaml` code block.** Content is never dropped.
- An empty block renders nothing.
- **A `---` anywhere other than the very start of the document is still a horizontal rule**, so existing pages look exactly as they did.

## Extensions via renderer plugins

The following syntax only works when the corresponding **renderer plugin** is
enabled. See [Installing and configuring plugins](../operations/plugins) for how to manage
plugins.
...
- Rendering requires a PlantUML server. The server URL is configured from
  the admin screen at `/admin/plugins` (see [Renderer plugins](../operations/renderers)
...
- Diagram source is limited to 20KB. A diagram with a syntax error shows a
  ...
  see the Mermaid section of [Renderer plugins](../operations/renderers).
...
- [Installing and configuring plugins](../operations/plugins) — enabling renderer plugins
- [Renderer plugins](../operations/renderers) — configuring renderer plugins
- [Markdown syntax](../reference/markdown) — the syntax cheat sheet
