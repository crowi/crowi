...
/// RFC-0016 §3 — `workspaceOrigin + "/api/v2"`. This is what the generated
...
/// `openapi.json` carries `/api/v2` in its `servers` entry while operation
/// `path`s are bare (`/pages`, …), so the generated client expects its
/// `serverURL` to already include `/api/v2`. Passing a `WorkspaceOrigin`
/// alone to `Client(serverURL:)` would send every call to the wrong path
/// (e.g. `https://host/pages` instead of `https://host/api/v2/pages`) and
...
public struct APIBaseURL: Equatable, Sendable, Hashable {
    ...
    public init(workspaceOrigin: WorkspaceOrigin) {
        self.url = workspaceOrigin.baseURL.appendingPathComponent("api/v2")
    }
}
...
extension APIBaseURL {
    ...
    /// `"/api/v2/..."` prefix, which is exactly the mistake this type
