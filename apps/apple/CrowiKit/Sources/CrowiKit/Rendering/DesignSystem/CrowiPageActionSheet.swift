...
public struct CrowiPageActionSheet: View {
    ...
    public var body: some View {
        ScrollView {
            VStack(spacing: CrowiMetrics.pageHeaderSpacing) {
                CrowiCard {
            ...
            .padding(.top, CrowiMetrics.pageHeaderSpacing)
        }
    ...
    /// Tall enough for the rows at the current text size, and never taller
    /// than the sheet can be — the `.large` companion detent (plus the
    /// `ScrollView` above) is what keeps an accessibility text size from
    /// clipping the last row instead of scrolling it.
    private var preferredHeight: CGFloat {
        rowHeight * CGFloat(CrowiPageAction.sheetActions.count) + rowHeight + CrowiMetrics.pageHeaderBottomPadding * 3
    }
