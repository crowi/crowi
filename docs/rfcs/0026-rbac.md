...
`RoleMembership` has a unique `(roleId, userId)` index and reverse indexes for membership listing and invalidation. `PageAccessPolicy.anchorPageId` has a partial unique index, with one separately identified default policy. `PageAccessGrant` has a unique `(policyId, policyRevision, principalType, principalId)` identity; the `linkClaimant` principal carries no id, so it uses a normalized sentinel or partial-index strategy. The `everyone` Role is an ordinary Role row and needs no such exception, but it is marked undeletable and its membership is resolved by rule rather than by `RoleMembership` rows. `PageLinkClaim` has a unique `(pageId, userId)` index plus a user-first index for access-context resolution. `UserAuthorizationState` has one row per user. Page has an index on `effectiveAccessPolicyId` composed with the status fields used by protected list queries, and a unique/conditional region-fence index supports creation and hierarchy operations.
...
The authoritative access version for `(page, actor)` is the serializable vector `{ pageId, pageAccessGeneration, effectivePolicyId, policyAuthorizationEpoch, actorId, actorAuthorizationGeneration, authorityVersion }`. `pageAccessGeneration` is stored on the Page and CASed with pointer, lifecycle, and authorization-path changes; `policyAuthorizationEpoch` is advanced atomically with the selected policy head; `actorAuthorizationGeneration` is advanced atomically with that actor's membership or exact-page claims; and `authorityVersion` is the version of the singleton authorization-cutover state read for the request. Capabilities and durable-write probes compare the complete vector.
...
A resolver reads the Page, selected policy head, active Role memberships, exact-page claims, actor generation, and authority version into one access vector, then re-reads the version owners before returning the context. A protected Page writer repeats that validation immediately before commit and predicates the Page write on the expected effective-policy pointer, Page generation, and absence of a Page transition. Policy-head replacement atomically advances the head revision, principal summary, and policy epoch; membership and exact-claim changes atomically update the actor state and actor generation; pointer and lifecycle operations advance the affected Page generation while holding a Page transition. Revokers never wait for readers and readers never occupy durable slots. A policy, actor, or authority change that lands after final validation but before the Page CAS may permit one stale-authorized commit; this bounded standalone-Mongo race is explicit, produces ordinary revision/history evidence, and does not apply to Page-local pointer or lifecycle changes.
...
Every mutation reauthorizes in the service/command layer immediately before its committing write. A Page mutation carries the complete §5 `(page, actor)` access vector, revalidates the policy epoch, actor generation, and authority version, and predicates its Page write on the Page generation, effective-policy pointer, and absence of a Page transition. A Page-local mismatch returns a conflict and performs no Page mutation. A policy, actor, or authority change in the bounded interval after revalidation is the explicit §5 residual window; commands do not create reader leases to eliminate that interval.
