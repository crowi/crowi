...
Notification selection derives watcher recipients and mention targets without a Page `READ` decision before creating notification rows (`packages/api/src/models/activity.ts:246-268`, `packages/api/src/models/activity.ts:272-300`). There is no outbound channel today, since `packages/api/src/service/notification.ts` leaves its mail and chat methods empty, so nothing unretractable is sent. The defect is that rows are created at all for a watcher who can no longer read the Page. Response-time filtering hides those rows rather than preventing them, and it would not survive the addition of a delivery channel.
