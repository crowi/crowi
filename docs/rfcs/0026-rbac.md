...
Each explicit access policy is attached to one anchor Page. Every Page stores a materialized pointer to its effective policy: its own explicit policy when one exists, otherwise the nearest explicit policy on a path ancestor, otherwise the singleton default policy. A fresh install binds the `everyone` Role at `MANAGE` in the root-scope policy, preserving Crowi's open-by-default posture; an administrator may change that binding like any other. An explicit child policy is a complete replacement, not an additive patch, so it can narrow or widen access and can remove the creator's access.
...
The installed channel-unfurl integration loads a Page with its populated Revision, but it already withholds content for anything other than a public Page: `buildUnfurlAttachment` returns a minimal card carrying no body, no path and no breadcrumb for restricted, specified and owner Pages (`packages/plugin-slack/src/unfurl.ts:73-86`), a deliberate data-leakage guard recorded at that call site. What remains is narrower. The minimal card still confirms that some Page exists behind the shared URL, and the guard keys off a duplicated `GRANT_PUBLIC` constant (`packages/plugin-slack/src/constants-grant.ts`) rather than the authorization service. Content-egress integrations therefore need a stated contract, not a missing guard supplied.

Notification selection derives watcher recipients without a Page `READ` decision before creating notification rows (`packages/api/src/models/activity.ts:246-268`, `packages/api/src/models/activity.ts:272-300`), and mention dispatch bypasses that selection entirely to upsert for the mentioned user directly (`packages/api/src/events/mention-dispatch.ts:138-155`). Every successful upsert then forwards the notification to each registered notifier driver (`packages/api/src/models/notification.ts:142-148`, `:319-338`), whose contract is explicitly provider egress such as chat posts and webhook requests (`packages/plugin-api/src/registries/notifier.ts:30-43`). Delivery is therefore already outbound and unretractable once sent, and the notification list serializes a populated target without re-checking the grant (`packages/api/src/hono/handlers/notification.ts:104-108`, `:143-165`). Recipients must be authorized at selection time, on both the watcher and the mention path, because no response-time filter can recall an external message.
...
- Preserve Crowi's open-by-default behavior by binding the implicit-membership `everyone` Role at `MANAGE` in the root scope on a fresh install, while letting an administrator change that binding.
...
An **anchor Page** owns an explicit `PageAccessPolicy`. An **inheriting Page** has no local policy and points to the nearest anchor policy. The **effective policy pointer** is the materialized Page reference used for authorization and indexing. A **binding** assigns one level to one Role, one user, or one system principal within one complete policy snapshot. `everyone` is an ordinary Role whose display metadata and policy bindings are administrator-editable, while its identity, implicit-membership rule, protected marker, and active status are immutable: every authenticated user matches it and no membership rows are stored for it. It cannot be deleted or deactivated, because a policy whose only binding names it would otherwise become unreachable. The one remaining system principal is exact-Page `linkClaimant`, which stays outside the Role model because its membership is per Page; a Role binding would extend it to every descendant sharing the policy.
...
The root-scope policy binds the `everyone` Role at `manage` on a fresh install, so a Page with no explicit policy remains fully open to authenticated users. An administrator may lower that binding; this is how a workspace whose sign-in audience is broader than its intended readership narrows its default without touching every Page. Lowering it to `none` would seal every inheriting Page in one edit, so it requires the same destructive confirmation as a zero-manager policy and stays recoverable through §9. An explicit policy may have only lower-level bindings or user `none` bindings and may consequently have no manager. An empty explicit binding set is invalid because “no configured page permission” canonicalizes to inheritance/default-open behavior rather than an accidental sealed Page.
...
erDiagram
  ...
  PAGE_ACCESS_GRANTS {
    ...
    ObjectId principalId FK "added; absent for the linkClaimant principal"
    string level "added"
  }
  PAGE_LINK_CLAIMS {
...
Immutable, revisioned binding rows make a policy replacement atomic to readers. Before writing grant rows, a producer claims an immutable snapshot manifest identified by `(policyId, policyRevision)` and containing its owner operation, expected predecessor revision, canonical binding digest, principal summary, and binding count. Only a producer matching that manifest may write the revision. After exact primary readback, the manifest advances once to complete; only a complete manifest may be published by a head CAS against its expected predecessor. Each published revision retains that immutable predecessor so a reclaimed operation can distinguish an applied-then-superseded write from a prepared-only snapshot. A losing writer cannot add rows to a revision another producer may publish. Unpublished manifests and grants remain unreachable and are eligible for garbage collection.

`RoleMembership` has a unique `(roleId, userId)` index and reverse indexes for membership listing and invalidation. `PageAccessPolicy.scopeKey` has an ordinary unique index. The canonical persisted scope is either `root` with no `anchorPageId`, or `page:<lowercase ObjectId hex>` with the same ObjectId in `anchorPageId`; the headed-insert API accepts a discriminated scope and derives both fields, and readers fail closed on a mismatched persisted pair. `PageAccessGrant` has a unique `(policyId, policyRevision, principalType, principalId)` identity; the `linkClaimant` principal carries no id, so it uses a normalized sentinel or partial-index strategy. The `everyone` Role is an ordinary Role row and needs no such exception, but it is marked undeletable and its membership is resolved by rule rather than by `RoleMembership` rows. `PageLinkClaim` has a unique `(pageId, userId)` index plus a user-first index for access-context resolution. `UserAuthorizationState` has one row per user. Page has an index on `effectiveAccessPolicyId` composed with the status fields used by protected list queries, and a unique/conditional region-fence index supports creation and hierarchy operations.
...
The authoritative access evidence for `(page, actor)` is a discriminated serializable vector. While runtime authority is legacy, the active variant is `{ authorityMode: 'legacy', pageId, pageAccessGeneration, effectivePolicyId, actorId, authorityVersion, legacyPageAuthorizationDigest }`, where `effectivePolicyId` may be null and `legacyPageAuthorizationDigest` is the canonical SHA-256 of the Page's `grant`, ordered `grantedUsers`, `status`, and `creator` decision inputs. No policy epoch or actor-model generation is fabricated when no such durable owner exists. Under model authority, the active variant is `{ authorityMode: 'model', pageId, pageAccessGeneration, effectivePolicyId, policyAuthorizationEpoch, actorId, actorAuthorizationGeneration, authorityVersion, pageLifecycleDigest }`; its `effectivePolicyId` is non-null and its policy epoch is a positive integer. A model vector is produced only when the effective policy pointer resolves to a current complete snapshot and all lifecycle evidence normalizes successfully. A pointerless Page, dangling pointer, null head, incomplete or corrupt snapshot, missing or malformed policy epoch, or malformed lifecycle evidence produces a fail-closed decision without an access vector; no zero or null policy epoch is fabricated and no capability may be minted from that result. A fully headed policy insert stores `authorizationEpoch: 1` atomically with `headRevision` and `headPrincipals`; each later successful head replacement increments it exactly once. `pageAccessGeneration` is stored on the Page and CASed with pointer, lifecycle, and authorization-path changes; `actorAuthorizationGeneration` is advanced atomically with that actor's membership or exact-page claims; and `authorityVersion` is the version of the singleton authorization-cutover state read for the request. Capabilities and durable-write probes serialize and compare only a complete active variant through one shared versioned wire schema.

Durable authorization operations and hierarchy-region fences have an owner identity, an expiry, renewal while held, and a fresh opaque lease token on every expired-lease reclamation. The current lease token gates operation renewal, progress, acquisition of new work, and terminal commits. A Page transition instead carries an immutable fence derived from the durable operation intent and expected Page source state. Page marker installation, domain writes, and clearing match that fence, the Page generation, and the required source or final state. A slow former holder may only replay the same convergent intent; it cannot commit a different state or clear an incomplete transition. Ordinary authorization readers and protected Page writers do not create durable reader claims; they use a version snapshot, final revalidation, and the Page-local commit predicate described below.
...
A resolver reads the Page-local decision inputs and, under model authority, the selected policy head, active Role memberships, exact-page claims, actor generation, and authority version into an access decision and, only when every owner required by the active variant is present and valid, one access vector; it then re-reads the applicable version owners and Page evidence before returning the context. A protected Page writer repeats that validation immediately before commit and predicates the Page write on the evaluated Page-local snapshot: the expected effective-policy pointer, Page generation, absence of a Page transition, the real-Page condition (`redirectTo` is null), `status`, and `creator`, plus `grant` and `grantedUsers` while legacy authority is active. Policy-head replacement atomically advances the head revision, principal summary, and policy epoch; membership and exact-claim changes atomically update the actor state and actor generation; pointer and lifecycle operations advance the affected Page generation while holding a Page transition. Revokers never wait for readers and readers never occupy durable slots. A policy, actor, or authority change that lands after final validation but before a Page CAS may permit that protected Page-write attempt to commit once under stale authorization. Multiple attempts on different Pages that completed final validation before the change may each commit; the protocol does not impose a deployment-wide one-commit bound. Every later attempt observes the new remote version. This bounded standalone-Mongo race is explicit and produces ordinary revision/history evidence. It does not apply to a Page-local decision input because every such input is pinned directly by the committing Page predicate or changed under the Page generation and immutable transition fence.

Role deletion is deactivation, never physical removal: the command is rejected while a current policy head references the Role or the Role has members, unless an explicit durable rewrite supplies the replacement/removal semantics. A Role that passes that check is made inactive in one atomic write (§7); its row and ID are retained under the `inactive:` tombstone identity, and references that arrive concurrently are deny-only and are surfaced by recovery inventory. Silent deletion is prohibited. Unreachable policy snapshots and policies flattened by a completed move are garbage-collected only after durable reference checks; they are never synchronously deleted in the command's correctness path.
...
Every mutation reauthorizes in the service/command layer immediately before its committing write. A Page mutation carries the complete active §5 access-vector variant, revalidates every remote version present in that variant, and predicates its Page write on the finally validated Page-local authorization and lifecycle snapshot: Page generation, effective-policy pointer, absence of a Page transition, the real-Page condition (`redirectTo` is null), `status`, `creator`, and, while legacy authority is active, `grant` and `grantedUsers`. A protected content writer separately captures the raw presence and value of `revision`, `currentRevision`, `path`, and the collaboration lifecycle version from the same initial Page snapshot used to derive its content. Final revalidation rejects the write if those base values changed or a history transition is active, and the committing Page predicate pins the captured base values plus the absence of a history transition rather than adopting values from the final re-read. A generic content command must not persist implicit or default-only changes to fields owned by history, collaboration, access, or topology protocols, and any explicit unauthorized delta to those fields is rejected before the write. Ordinary read decisions do not compare content pointers, paths, history state, or collaboration epochs because those values are write-fencing evidence rather than ordinary read-authorization evidence. A Page-local mismatch returns a conflict and performs no Page mutation. Revision staging remains inactive until one readiness gate proves that every canonical Page-pointer writer publishes staged base pointers before superseding them, stages its own candidate before the base-pinned Page CAS, publishes an accepted candidate afterward, and every caller-specified or page-scan Revision reader enforces reachability. Once a Page CAS durably makes a candidate Revision canonical, that Revision remains reachable from history and detail regardless of response delivery, stage publication, history allocation, later writers, or process failure; garbage collection may reclaim only a stage proven never to have become canonical. Activation requires a drained homogeneous fleet, and model-authority cutover is rejected while any registered writer or reader closure is absent. A policy, actor, authority, active-Role, or active-User change in the bounded interval after revalidation is the explicit §5 per-attempt residual window; commands do not create reader leases to eliminate that interval.
...
A membership change updates one `UserAuthorizationState` document with a compare-and-set that changes the membership set and advances its generation atomically. A multi-user administrative command records fixed intent and progress in a durable authorization operation and applies those per-actor writes idempotently; it is not reported complete until its audit and invalidation obligations finish. Role deletion first makes a non-protected Role inactive in one atomic write, after which resolvers ignore it, and then resumes reference cleanup. Inactive is terminal: a Role ID is never reactivated, and recreating a logical Role uses a new ID. Concurrent late references are therefore deny-only and are reported by recovery inventory rather than protected by a shared Role reader-lease protocol.
...
Zero-manager policies are permitted after an explicit destructive confirmation that names the recovery consequence. The sole exception is fresh root bootstrap: when the root policy is absent, the trusted boot path may publish exactly the canonical single `everyone: manage` binding without an active-user witness or public bypass option. The creator receives no implicit rescue grant. The administrator recovery command in §9 is therefore a required invariant, not an optional operational convenience.
...
Administrators create, rename, order, describe, and delete (deactivate, per §5) Roles and manage memberships through typed `/admin` contracts protected by `createJwtAdminRequired`. That middleware already requires both a web-session auth context and `user.admin === true`; an administrator's PAT or OAuth token is rejected (`packages/api/src/hono/middleware/admin.ts:38-59`). The same boundary is retained.

A Role has identity and presentation metadata but no server-administrator bit and no global page-permission level. Its unique identity name is `displayName.normalize('NFKC').trim().toLowerCase()`; the `everyone` identity and the `inactive:` tombstone namespace are reserved. Administration UI shows member count and policy usage, including the binding level at each usage, so the impact of a role edit or deactivation is reviewable. Membership revocation advances its authorization generation and invalidates affected active collaboration connections; it does not wait for JWT expiry.
...
Before a policy, membership, user override, link-claim, pointer, or lifecycle revocation is reported complete, the command advances the relevant durable vector component and attempts the typed invalidation for every affected Page or actor. Invalidation delivery accelerates tombstoning, detach, and force reload but is not the durable correctness boundary across replicas.

Every appended collaboration update persists the authenticated actor and complete access vector. Replay, count-triggered compaction, store-triggered checkpointing, and document loading reauthorize `EDIT` for each distinct evidence tuple and reject evidence that is missing or no longer matches. Rejected rows are never promoted to `Page.yjsState` or Revision.

Every durable collaboration save separately reauthorizes `EDIT`, compares policy, actor, and authority versions immediately before commit, and predicates its Page CAS on the expected Page generation, effective-policy pointer, and absence of a Page transition. Cross-replica invalidation may be delayed or lost, and an already-merged live Y.Doc may remain visible until drain, but stale updates cannot become canonical.
...
Storing one page level on Role is rejected because a Role reused across policies could not express local authority, and an administrator Role edit could silently change every Page. A Role remains identity plus membership; each policy binding owns its level. The `everyone` Role makes this concrete: the same Role must be able to hold `manage` at the root and `read` under a narrowed branch, which a Role-global level cannot express.
...
- Add the complete access vector and version-validation protocol to tokens and saves, full `onAuthenticate` reauthorization, read-only issuance, active connection invalidation, cross-replica delivery, readiness gating, and stale-update refusal.
...
- Every operation and hierarchy-region lease expires, renews while held, and is reclaimed only with a fresh lease token. A former holder cannot renew, claim new work, advance operation progress, or finalize the operation with its old token. Page transitions use the immutable intent fence and Page-local source/final-state CAS: an old holder may only replay the same convergent transition, cannot write a different result, and cannot clear an incomplete transition. Ordinary authorization readers and protected Page writers hold no lease and therefore cannot be starved by one.
...
Options were (1) binding-level only, (2) Role default with policy override, and (3) Role-global only. This RFC adopts option 1 because it keeps membership reusable and makes authority local and reviewable. **Confirmed.** The `everyone` Role settles it: a single Role has to hold different levels in different scopes, which only a binding-level design expresses. It also keeps an externally synchronised Role limited to stating who belongs to a group, leaving the authority that group carries under local control.
