...
Durable authorization operations and hierarchy-region fences have an owner identity, an expiry, renewal while held, and a fresh opaque fencing token on every expired-lease reclamation. Their Page transition writes, clears, and terminal commits must match the current token. Ordinary authorization readers and protected Page writers do not create durable reader claims; they use a version snapshot, final revalidation, and the Page-local commit predicate described below. An expired operation lease may be reclaimed with a fresh token, so a slow former holder cannot mutate or clear Page transitions after reclamation.
...
- Every operation and hierarchy-region lease expires, renews while held, is reclaimed only with a fresh fencing token, and rejects a former holder's late Page transition write, clear, or finalization against the current token. Ordinary authorization readers and protected Page writers hold no lease and therefore cannot be starved by one.
