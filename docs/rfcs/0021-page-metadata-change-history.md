...
Every Page whose history tracking has started (`ready`) receives its own `page_renamed` event and sequence; `subtree: true` plus the shared operationId lets the UI identify the operation group. A Page whose tracking has not started (`untracked`) is moved the same way but produces no event — there is no history line yet to append to. A partial failure never erases events for Pages already moved.
...
**Member result authority.** A member's move commits in three writes, and only the first is the fact. The exit CAS moves the Page and stages the event in its outbox (`pendingHistoryEntry`) in one update — that is the commit. The grouped `PageHistoryEvent` is that entry materialized, several round-trips later, and `PageHistoryOperation.result` is a cache of the verdict, later still. Evidence of completion is therefore the staged entry OR the event row, never the row alone: between the CAS and the row there is a window in which the move has landed and no row exists yet. A command that fans a request out across per-member records — today, subtree rename is the only one — must read them in that order and never the reverse. Reading a later write as absent and reporting the member failed mistakes lag for absence, and the mistake is not cosmetic: the caller is told a subtree move failed and may have moved only part of the tree, when every page moved and every event landed. Before any such command reports a member as failed, it takes one primary-pinned Page snapshot, settles the member `succeeded` if that snapshot carries the staged entry or if the event row exists (materializing the staged entry on the committer's behalf so later readers see the row), and classifies the same snapshot for the failure path so the two decisions cannot straddle the commit; only when the evidence check itself cannot be performed (the operation record could not be read) does it fall back to reporting failure, because asserting success without evidence is worse than a failure a later repair sweep can still correct. A command with a single operation per Page — every non-subtree command in this RFC — has no member results to build this window around and is unaffected; its resume logic already checks event evidence directly.

An untracked Page has no event to check at all, so its position is the only evidence that can exist: a member whose transition is released and whose Page sits at the record's `toPath` is accepted as settled, the same rule a single-page rename's own resume logic already applies. This is not a relaxation of the principle above — asserting success without evidence is still worse than a failure a later repair sweep can correct — because for an untracked Page there is no event to omit checking; position is the whole of what a completed move can ever leave behind. A tracked Page is never settled by position — its event can exist, so its absence means the move has not committed, and the staged-entry-or-event-row rule above still governs it exclusively. One record is excluded from the untracked exception: one whose `fromPath` equals its `toPath` never named a move, so the Page's position cannot prove it — sitting at that single path is indistinguishable from never having been touched — and such a record always requires the event evidence above, which an untracked Page can never produce, so it never resolves by position.
