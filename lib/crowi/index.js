...
Crowi.prototype.init = function() {
  ...
  return Promise.resolve()
    ...
    .then(function() {
      self.setupRedisClient()
    })
    .then(function() {
      return self.setupSessionConfig()
    })
    .then(function() {
      return self.setupConfigPubSub()
    })
    .then(async function() {
...
Crowi.prototype.beforeSetConfig = function(config) {
  if (this.publisher) {
    const { pubSubChannel, pubSubId } = this
    this.publisher.publish(pubSubChannel, JSON.stringify({ pubSubId, config }))
  }
}

Crowi.prototype.setConfig = function(config) {
  this.beforeSetConfig(config)
  this.config = config
}
...
Crowi.prototype.setupConfigPubSub = function() {
  if (this.redisOpts) {
    ...
    if (this.subscriber) {
      ...
      this.subscriber.subscribe(this.pubSubChannel)
    }
  }
}
...
Crowi.prototype.setupRedisClient = async function() {
  if (this.redisOpts) {
    const redis = require('redis')
