...
SearchClient.prototype.filterPagesByUser = function(query, user) {
  const Page = this.crowi.model('Page')
...
SearchClient.prototype.searchKeyword = function(keyword, user = {}, option = {}) {
...
SearchClient.prototype.searchKeywordUnderPath = function(keyword, path, user = {}, option = {}) {
  const from = option.offset || null
  const size = option.limit || null
  const type = option.type || null
  const query = this.createSearchQuerySortedByScore()
