module.exports = function(crowi, app, req, locals) {
  ...
  locals.isOwner = function(user, page) {
    if (!page.owners) return false
    const ownerUsers = []
    page.owners.forEach(owner => ownerUsers.push(...owner.team.users))
    return ownerUsers.filter(ownerUser => ownerUser._id.equals(user._id)).length > 0
  }
