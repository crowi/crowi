...
exports.fileAccessRightOrLoginRequired = function(crowi, app) {
  return async function(req, res, next) {
    try {
      ...
      const share = await Share.findShareByPageId(attachment.page, Share.STATUS_ACTIVE)
      const { shareIds = [], secretKeywords = {} } = req.session
      console.log(secretKeywords, share.secretKeyword)
      const matchPassword = !share.secretKeyword || secretKeywords[share.id] === share.secretKeyword
      if (matchPassword && shareIds.includes(share.id)) {
        return next()
      }
    } catch (err) {
      debug(err)
