module.exports = function(crowi) {
  ...
  activitySchema.statics.removeByParameters = function(parameters) {
    ...
    return new Promise(async function(resolve, reject) {
      try {
        const activities = await Activity.find(parameters)
  ...
  activitySchema.methods.getNotificationTargetUsers = function() {
    ...
    return new Promise(function(resolve, reject) {
      self.model(self.targetModel).findById(self.target, function(err, model) {
        if (err) {
          reject(err)
        }
  ...
  activitySchema.post('save', function(savedActivity) {
    ...
    Promise.all([savedActivity.getNotificationTargetUsers(), savedActivity.getSameActivityUniqueUsers()])
      .then(function(results) {
        ...
        notificationTargetUsers.forEach(function(user) {
          ...
          notificationPromises.push(Notification.upsertByActivity(user, filteredUsers, savedActivity))
        })
  ...
  activityEvent.on('remove', async function(activity) {
    const Notification = crowi.model('Notification')
    const { user: actionUser } = activity

    try {
      const [notificationTargetUsers, sameActivityUsers] = await Promise.all([activity.getNotificationTargetUsers(), activity.getSameActivityUniqueUsers()])
      const notificationPromises = notificationTargetUsers
        .filter(user => {
        ...
        .map(user => {
          const filteredUsers = sameActivityUsers.filter(sameActionUser => user.toString() !== sameActionUser.toString()).filter(sameActionUser => {
            // Remove action user from notification when activity removed
            return actionUser.toString() !== sameActionUser.toString()
          })
          return Notification.removeByActivity(user, filteredUsers, activity)
        })
