module.exports = function(crowi) {
  ...
  activitySchema.statics.removeByParameters = function(parameters) {
    ...
    return new Promise(function(resolve, reject) {
      try {
        ...
        activityEvent.emit('remove', Activity, parameters)
      } catch (e) {
        reject(e)
      }
    })
  ...
  activityEvent.on('remove', function(Activity, parameters) {
    const Notification = crowi.model('Notification')
    const NotificationStatus = crowi.model('NotificationStatus')

    const params = {}
    Notification.removeIfExists(params)
      .then(d => {})
      .catch(err => debug('Remove Notification error', err))
  })
