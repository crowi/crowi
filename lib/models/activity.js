module.exports = function(crowi) {
  ...
  activitySchema.post('save', async function(savedActivity) {
    ...
    const { target } = savedActivity
    try {
      const [targetUsers, watchUsers, ignoreUsers, sameActivities] = await Promise.all([
        savedActivity.getNotificationTargetUsers(),
        Watcher.getWatchers(target),
        Watcher.getIgnorers(target),
        savedActivity.getSameActivities(),
      ])
      ...
      const watchers = filter(unique([...targetUsers, ...watchUsers]), ignoreUsers)

      const notificationPromises = watchers.map(user => {
