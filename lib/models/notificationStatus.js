module.exports = function(crowi) {
  'use strict';

  const debug    = require('debug')('crowi:models:notification-status');
  const mongoose = require('mongoose');
  const ObjectId = mongoose.Schema.Types.ObjectId;

  const notificationStatusSchema = new mongoose.Schema({
    user: {
      type: ObjectId,
      ref: 'User',
      index: true,
      require: true,
    },
    count: {
      type: Number,
      require: true,
      default: 0,
    },
    is_read: {
      type: Boolean,
      default: false,
    },
  });

  notificationStatusSchema.statics.findByUser = function(user) {
    const NotificationIndicator = this;

    return new Promise(function(resolve, reject) {
      const query = {
        user: user,
      };

      NotificationIndicator
        .findOne(query)
        .exec(function(err, notificationStatus) {
          if (err) {
            reject(err);
          }

          resolve(notificationStatus);
        })
      ;
    });
  };

  notificationStatusSchema.statics.upsertByNotification = function(user, count) {
    const NotificationIndicator = this;

    return new Promise(function(resolve, reject) {
      const query = {
        user: user,
      };

      const parameters = {
        user: user,
        count: count,
        is_read: false,
      };

      const options = {
        upsert: true,
        new: true,
        setDefaultsOnInsert: true,
      };

      NotificationIndicator.findOneAndUpdate(query, parameters, options, function(err, notificationStatus) {
        if (err) {
          reject(err);
        }

        crowi.getIo().sockets.emit('notification updated', {status: notificationStatus});

        debug(notificationStatus);
        resolve(notificationStatus);
      });
    });
  };

  notificationStatusSchema.statics.updateIsReadFlag = function(user, flag) {
    flag = flag || false;

    const NotificationIndicator = this;

    return new Promise(function(resolve, reject) {
      const query = {
        user: user,
      };

      const parameters = {
        is_read: flag,
      };

      const options = {
        upsert: false,
        new: true,
      };

      NotificationIndicator
        .findOneAndUpdate(query, parameters, options)
        .exec(function(err, notificationStatus) {
          if (err) {
            reject(err);
          }

          resolve(notificationStatus);
        })
      ;
    });
  };

  // TODO: status の更新内容を考える
  // - Notification が追加された: is_read を false に。count を更新
  // - Status がクリックされた: is_read を true に。count はそのまま
  // - 個別の Notification がクリックされた: is_read は変更せず、 count を更新
  // - /me/notifications ページをみた: is_read は true に。count はそのまま

  return mongoose.model('NotificationIndicator', notificationStatusSchema);
}
