module.exports = function(crowi) {
  ...
  notificationSchema.statics.removeByActivity = async function(user, sameActivityUsers, activity) {
    const Notification = this
  ...
  notificationSchema.statics.removeEmpty = function() {
    const Notification = this
    return Notification.findOneAndRemove({ latestActionUsers: { $size: 0 } })
  }
  ...
  notificationSchema.statics.upsertPostHook = function(user) {
    ...
    Promise.resolve(Notification.getUnreadCountByUser(user))
      .then(function(count) {
        debug('unread count', count)
        return NotificationStatus.upsertByNotification(user, count)
      })
