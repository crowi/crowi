module.exports = function(crowi) {
  ...
  pageSchema.statics.updatePage = async function(pageData, body, user, options = {}) {
    ...
    const expirationAt = await (async () => {
      if (options.bodyConfirmed) {
      ...
      if (pageData.lifetime) {
        // no need to require high-accuracy time
        return moment()
          .add(pageData.lifetime)
          .endOf('day')
          .toDate()
      }
      return pageData.populated('revision') ? pageData.revision : null
    })()
