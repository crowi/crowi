module.exports = function(crowi) {
  ...
  pageSchema.methods.getNotificationTargetUsers = function() {
    var Comment = crowi.model('Comment')
    var Revision = crowi.model('Revision')
    var self = this

    return Promise.all([Comment.findCreatorsByPage(self), Revision.findAuthorsByPage(self)]).then(function(results) {
      var commentCreators = results[0]
      var revisionAuthors = results[1]

      debug('commentCreators', commentCreators)
      debug('revisionAuthors', revisionAuthors)

      var targetUsers = [self.creator]
      targetUsers = targetUsers.concat(commentCreators, revisionAuthors)
