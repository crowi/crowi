module.exports = function(crowi) {
   ...
   */
  pageSchema.statics.calculationRevisionExpirationAt = async ({ pageId, userId, bodyConfirmed, revision = null, lifetime = null } = {}) => {
  ...
  pageSchema.statics.updatePage = async function(pageData, body, user, options = {}) {
    ...
    const expirationAt = await Page.calculationRevisionExpirationAt({
    ...
    try {
      ...
      // FIXME: 以前の実装では、revision が更新されていない、引数の pageData をここで返却していたが
      // 更新すべきじゃないか？とおもったため、そうした。問題ないか？
      pageEvent.emit('update', savedPage, user, bookmarkCount)
      return savedPage
    } catch (err) {
      // FIXME: 現在, ページ更新でエラーが起きても利用者が検知できない状況にあって、望ましくないと感じている
