module.exports = function(crowi) {
  ...
  const pageSchema = new mongoose.Schema(
    {
      ...
      updatedAt: Date,
      // If this is set, revision's expireAt will be set
      lifetime: {
        type: String,
        get: function(data) {
          try {
  ...
  /**
   * Calculate next revision's expireAt
   * this method knows revision model.
   * @returns {Promise<Date|null>}
   */
  pageSchema.statics.calculateRevisionExpireAt = async ({ pageId, userId, bodyConfirmed = false, revision = null, lifetime = null } = {}) => {
    const Team = crowi.model('Team')
    const PageOwner = crowi.model('PageOwner')

    // Inherit expiration date from previous revision
    if (!bodyConfirmed && revision && revision.expireAt) {
      return revision.expireAt
    }

    // If user confirmed body, check whether it's in owner teams.
    if (bodyConfirmed) {
      const teams = await Team.findByUser(userId).populateAll()
      if (teams.length === 0) throw new Error('Only owners can set expireAt flag.')
      const po = await PageOwner.findByTeam(...teams)
        .findByPage(pageId)
        .exec()
      if (po.length === 0) throw new Error('Only owners can set expireAt flag.')
    }

    // When (No target to inherit || User confirmed) && lifetime set, lifetime used to determine expireAt.
    if (lifetime) {
  ...
  pageSchema.statics.updatePage = async function(pageData, body, user, options = {}) {
    ...
    const expireAt = await Page.calculateRevisionExpireAt({
    ...
    const newRevision = Revision.prepareRevision(pageData, body, user, { expireAt })
