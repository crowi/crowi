module.exports = function(crowi) {
  ...
  const pageSchema = new mongoose.Schema(
    {
      ...
      updatedAt: Date,
      // If this is set, revision's expirationAt will be set
      lifetime: {
        type: String,
        get: function(data) {
          try {
        ...
        set: function(data) {
          if (data === null) return null // user can reset setting
          return JSON.stringify(data)
        },
      },
    },
  ...
  pageSchema.methods.isUserPage = function() {
    return isUserPage(this.path)
  }

  pageSchema.methods.canBeOwned = function() {
  ...
  /**
   * @param {null, object} lifetime
   */
  pageSchema.methods.updateLifetime = async function(lifetime, user) {
    const Team = crowi.model('Team')
    const PageOwner = crowi.model('PageOwner')

    /**
     * must be object, approved fields can be found,
     * and no required field.
     **/
    ...
    let page = this
    if (!this.populated('owners')) {
      page = await page.populate('owners')
    }

    if (value !== null && page.owners && page.owners.length > 0) {
      throw new Error(`You can't set lifetime because target page isn't owned.`)
    }

    const teams = await Team.findByUser(user)
    const owner = await PageOwner.findByTeam(...teams)
      .findByPage(page)
      .findOne()

    if (!owner) throw new Error(`You can't set lifetime because you are not owner.`)
  ...
  pageSchema.statics.populatePageData = function(pageData, revisionId) {
    ...
    return pageData
      .populate([
        { path: 'lastUpdateUser', select: User.USER_PUBLIC_FIELDS },
        { path: 'creator', select: User.USER_PUBLIC_FIELDS },
        { path: 'revision', populate: { path: 'author', select: User.USER_PUBLIC_FIELDS } },
        {
          path: 'owners',
          match: { isActive: true },
          select: '_id team page',
          populate: {
            path: 'team',
            select: '_id users handle name',
            populate: {
              path: 'users',
  ...
  /**
   * Calculate next revision's expirationAt
   * this method knows revision model.
   * @returns {Promise<Date|null>}
   */
  pageSchema.statics.calculateRevisionExpirationAt = async ({ pageId, userId, bodyConfirmed = false, revision = null, lifetime = null } = {}) => {
    const Team = crowi.model('Team')
    const PageOwner = crowi.model('PageOwner')

    // Inherit expiration date from previous revision
    if (!bodyConfirmed && revision) {
      return revision.expirationAt
    }

    // If user confirmed body, check whether it's in owner teams.
    if (bodyConfirmed) {
      const teams = await Team.findByUser(userId).populateAll()
      if (teams.length === 0) throw new Error('Only owners can set expirationAt flag.')
      const po = await PageOwner.findByTeam(...teams)
        .findByPage(pageId)
        .exec()
      if (po.length === 0) throw new Error('Only owners can set expirationAt flag.')
    }

    // When (No target to inherit || User confirmed) && lifetime set, lifetime used to determine expirationAt.
    if (lifetime) {
      // no need to require high-accuracy time
      // Note: this behavior changes with machine timezone. We might need to fix this at future.
      return moment()
        .add(lifetime)
        .endOf('day')
        .toDate()
  ...
  pageSchema.statics.create = async function(path, body, user, options) {
    ...
    const pageData = await Page.findOne({ path: path })
    if (pageData) throw new Error('Cannot create new page to existed path')
    ...
    try {
      const data = await Page.pushRevision(savedPage, newRevision, user)
  ...
  pageSchema.statics.updatePage = async function(pageData, body, user, options = {}) {
    ...
    const bodyConfirmed = options.bodyConfirmed

    const expirationAt = await Page.calculateRevisionExpirationAt({
      revision: pageData.populated('revision') ? pageData.revision : null,
      lifetime: pageData.lifetime,
      bodyConfirmed,
      pageId: pageData._id,
      userId: user._id,
    })
    ...
    const newRevision = Revision.prepareRevision(pageData, body, user, { expirationAt })

    try {
      const savedPage = await Page.pushRevision(pageData, newRevision, user)

      const bookmarkCount = await Bookmark.countByPageId(pageData._id)

      // If need to update grant
      if (grant != pageData.grant) {
        const p = await Page.updateGrant(savedPage, grant, user)
        pageEvent.emit('update', p, user, bookmarkCount)
        return p
      }

      pageEvent.emit('update', savedPage, user, bookmarkCount)
      return savedPage
    } catch (err) {
