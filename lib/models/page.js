module.exports = function(crowi) {
  ...
  pageSchema.statics.completelyDeletePage = async function(pageData, user, options) {
    ...
    await Mention.removeByPage(pageId)
  ...
  pageSchema.methods.getNotificationTargetUsers = async function() {
    ...
    const revisionAuthors = await Revision.findAuthorsByPage(this)
    debug('revisionAuthors', revisionAuthors)

    const targetUsers = ObjectIdsUtil.unique([creator, ...revisionAuthors])
    debug('targetUsers', targetUsers)

    return targetUsers
  }

  const eventNames = ['create', 'update']
  eventNames.map(eventName => {
    pageEvent.on(eventName, async page => {
      const Backlink = crowi.model('Backlink')
      const Mention = crowi.model('Mention')

      let result

      result = await Backlink.createBySavedPage(page).catch(err => err)
      debug(result)

      result = await Mention.upsertByPage(page).catch(debug)
      debug(result)
    })
  })
