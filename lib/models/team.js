...
schema.methods.ownPage = async function(page) {
  ...
  return owner !== null // FIXME: すでに own してある (すでに) ケースでも true になる気がするがよいか?
}
...
schema.methods.disownPage = async function(page) {
  const { PermissionError } = this.constructor.crowi().errors
  ...
  // FIXME: 直感に反しないか？単に, own している事実がなかっただけなので PermissionError じゃない気がする
  if (owner === null) throw new PermissionError()
