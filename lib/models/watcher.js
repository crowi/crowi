module.exports = function(crowi) {
  ...
  const WatcherDefine = require('../util/activityDefine')()
  ...
  const watcherSchema = new mongoose.Schema({
    ...
    targetModel: {
      type: String,
      require: true,
      enum: WatcherDefine.getSupportTargetModelNames(),
    },
    target: {
      type: ObjectId,
      refPath: 'targetModel',
  ...
  watcherSchema.statics.getWatcherBrokerDocument = function(activity) {
    const { action, target, event } = activity

    const targetType = WatcherDefine.getTargetType(action)

    switch (targetType) {
      case WatcherDefine.TARGET_TYPES.TARGET:
        return target
      case WatcherDefine.TARGET_TYPES.EVENT:
        return event
      default:
        return null
    }
