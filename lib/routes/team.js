...
api.post('/teams/ownPage', async (req, res) => {
  const { id, page } = req.body

  const team = await Team.findById(id)
  if (!team) return res.status(404).json(ApiResponse.error('There are no team with given id.'))

  const p = await Page.findById(page).select('_id')

  const s = await team.ownPage(p)
  return res.json(s ? ApiResponse.success() : ApiResponse.error())
})

api.post('/teams/disownPage', async (req, res) => {
  const { id, page } = req.body
  ...
  const s = await team.disownPage(p) // FIXME: see error
  return res.json(s ? ApiResponse.success() : ApiResponse.error())
})
