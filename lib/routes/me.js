module.exports = function(crowi, app) {
  ...
  actions.authGitHub = function(req, res, next) {
    ...
    var toConnect = req.body.connectGitHub ? true : false;
    if (toDisconnect) {
      ...
      req.session.githubCallbackAction = '/me/auth/github/callback';
  ...
  actions.authGitHubCallback = function(req, res, next) {
    ...
    githubAuth.handleCallback(req, res, next)(function(err, tokenInfo) {
      if (err) {
        req.flash('warningMessage.auth.github', err);
        return res.redirect('/me'); // TODO Handling
      }

      var githubId = tokenInfo.user_id;
      var githubEmail = tokenInfo.email;

      if (!User.isEmailValid(githubEmail)) {
