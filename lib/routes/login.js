  ...
  actions.loginGoogle = function(req, res) {
    ...
    if (!code) {
      ...
      googleAuth.handleCallback(req, async (err, tokenInfo) => {
        ...
        const userData = await User.findUserByGoogleId(googleId).catch(err => {
  ...
  actions.loginGitHub = function(req, res, next) {
    ...
    if (!code) {
      ...
      githubAuth.handleCallback(req, res, next)(async (err, tokenInfo) => {
        ...
        const userData = await User.findUserByGitHubId(githubId).catch(err => {
  ...
  actions.register = function(req, res, next) {
    ...
    if (req.method == 'POST' && req.form.isValid) {
        ...
        User.createUserByEmailAndPassword(name, username, email, password, lang, async function(err, userData) {
          if (err) {
            ...
            if (googleId || githubId) {
              if (googleId) {
                userData = await userData.updateGoogleId(googleId).catch(err => {
                  // TODO
                })
              ...
              if (githubId) {
                userData = userData.updateGitHubId(githubId).catch(err => {
                  // TODO
                })
      ...
      const registerFailure = message => {
        ...
        const redirectUrl = isDisabledPasswordAuth ? '/login' : '/login?register=1'
        const type = isDisabledPasswordAuth ? 'warningMessage' : 'registerWarningMessage'
