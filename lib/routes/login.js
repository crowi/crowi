module.exports = function(crowi, app) {
  ...
  actions.googleCallback = function(req, res) {
    ...
    const { query } = req
    const { code = '' } = query
    const { googleSession = {} } = req.session
    const { callbackAction: nextAction = '/login' } = googleSession
    debug('googleCallback.nextAction', nextAction)
    req.session.googleSession = { authCode: code }
  ...
  actions.loginGoogle = function(req, res) {
    ...
    const { googleSession = {} } = req.session
    const { authCode: code } = googleSession
    ...
    if (!code) {
      googleAuth.createAuthUrl(req, function(err, redirectUrl) {
        ...
        req.session.googleSession = { callbackAction: '/login/google' }
  ...
  actions.register = function(req, res, next) {
    ...
    if (req.method == 'POST' && req.form.isValid) {
      ...
      User.isRegisterable(email, username, function(isRegisterable, errOn) {
        ...
        if (!User.isEmailValid(email)) {
          ...
          req.flash(
            ...
            'This email address could not be used. (Make sure the allowed email address)',
          )
        }
        if (!isRegisterable) {
          if (!errOn.username) {
                    ...
                    mailer.send(
                      {
                        to: adminUser.email,
                        subject:
                          '[' + config.crowi['app:title'] + ':admin] A New User Created and Waiting for Activation',
      ...
      const { googleSession = {}, github = {} } = req.session
      const { id: googleId } = googleSession
      const { id: githubId } = github
      const socialId = googleId || githubId
      const socialEmail = googleSession.email || github.email
      const socialName = googleSession.name || github.name
      const socialImage = googleSession.image || github.image
      ...
      if (github.organizations && !User.isGitHubAccountValid(github.organizations)) {
        req.flash(
          'registerWarningMessage',
          'This account could not be used. (Make sure whether you belong to allowed GitHub Organization)',
        )
        return res.redirect('/login?register=1')
      }
  ...
  actions.registerGoogle = function(req, res) {
    ...
    if (!code) {
      googleAuth.createAuthUrl(req, function(err, redirectUrl) {
        ...
        req.session.googleSession = { callbackAction: '/register/google' }
        return res.redirect(redirectUrl)
      })
    } else {
      ...
      googleAuth.handleCallback(req, function(err, tokenInfo) {
        ...
        req.session.googleSession = { id, email, name, image }
