module.exports = function(crowi, app) {
  ...
  // TODO: pageutil みたいなところにうつしててすとかく
  function getViewablePath(path) {
    return path.replace(/\s/g, '+')
  }

  function isRedirectToViewablePath(path) {
    debug('path', path)
    if (path.match(/\s/)) {
      return true
    }
  ...
  actions.userPageShow = async function(req, res) {
    ...
    res.locals.path = getViewablePath(path)
    debug('path', path)
  ...
  actions.pageShow = function(req, res) {
    if (isRedirectToViewablePath(req.params[0])) {
      return res.redirect(getViewablePath(req.params[0]))
    }
