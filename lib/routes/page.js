module.exports = function(crowi, app) {
  ...
  api.checkTreeRenamable = async function(req, res){
    const { page_id, new_path } = req.body;
    const newPagePath = Page.normalizePath(new_path);
    const page = await Page.findPageById(page_id).catch(err => {
      return res.json(ApiResponse.error('指定したページが存在しません'));
    });
    const pagePath = page.path;

    const paths = await Page.findListByStartWith(pagePath, req.user, {});
    const path_map = getPathMap(paths, pagePath, newPagePath);;

    const [error, errors] = await checkPagesRenamable(path_map);
