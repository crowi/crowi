module.exports = function(crowi, app) {
  ...
  var renderRecentList = function(template, req, res, options) {
    var username = req.params.username;
    var limit = 50;
    var offset = parseInt(req.query.offset) || 0;
    ...
    var pagerOptions = { sort: sort, offset: offset, limit : limit };
    var queryOptions = { sort: sort, offset: offset, limit : limit + 1};
    ...
    .then(function(user) {
      ...
      return res.render(template, renderVars);
    }).catch(function(err) {
      debug('Error on rendereing ' + template, err);
      res.redirect('/');
    });
  };

  // app.get( '/users/:username([^/]+)/recent-edit' , loginRequired(crowi, app) , page.userRecentEditedList);
  actions.userRecentEditedList = function(req, res) {
    var options = {
      sort: {updatedAt: -1}
    };
    renderRecentList('user/recent-edit', req, res, options);
  };
  ...
  actions.userRecentCreatedList = function(req, res) {
    ...
    renderRecentList('user/recent-create', req, res, options);
  };
