module.exports = function(crowi, app) {
  ...
  api.renameTree = async function(req, res) {
    ...
    const pathMap = getPathMap(paths, path, newPagePath)

    const [error, errors] = await checkPagesRenamable(pathMap, req.user)
    ...
    const rename = async function(oldPath, newPath) {
      try {
        ...
        const options = { createRedirectPage: !newPageIsPortal && createRedirect }
    ...
    try {
      const result = await Promise.all(
        Object.entries(pathMap).map(async function([oldPath, newPath]) {
