module.exports = function(crowi, app) {
  ...
  api.rename = function(req, res){
    ...
    if (!Page.isCreatableName(newPagePath)) {
      return res.json(ApiResponse.error(sprintf('You can\'t create page with that name (%s).', newPagePath)));
    }

    Page.findPageByPath(newPagePath)
    .then(function(page) {
      // if page found, cannot cannot rename to that path
      return res.json(ApiResponse.error(sprintf('You can\'t create page with that name (%s). Page exists.', newPagePath)));
    }).catch(function(err) {
