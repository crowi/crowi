module.exports = function(crowi, app) {
  ...
  actions.pageEdit = function(req, res) {
    ...
    var bodyConfirmed = pageForm.bodyConfirmed === 'true' ? true : false
    ...
    Page.findPage(path, req.user, null, ignoreNotFound)
      .then(function(data) {
        ...
        if (data) {
          ...
          return Page.updatePage(data, body, req.user, { grant: grant, bodyConfirmed })
        } else {
          // new page
  ...
  /**
   * @api {post} /pages.updateLifetime List pages by user
   * @apiName UpdateLifetime
   * @apiGroup Page
   *
   * @apiParam {String} page_id Target page's ObjectId.
   * @apiParam {Object|null} lifetime
   */
  api.updateLifetime = async function(req, res) {
    const { page_id, lifetime } = req.body

    try {
      const page = await Page.findPageById(page_id)
      await page.updateLifetime(lifetime, req.user)
    } catch (e) {
      return res.status(400).json(ApiResponse.error('Failed to watch this page.'))
    }
  }
