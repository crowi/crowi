module.exports = function(crowi, app) {
          ...
          if (notify.slack.on && notify.slack.channel) {
            ...
            if (crowi.slack) {
              notify.slack.channel.split(',').map(function(chan) {
                var message = crowi.slack.prepareSlackMessage(
                  pageData,
                  req.user,
                  chan,
                  updateOrCreate,
    ...
    Page.findPageByPath(newPagePath)
      .then(function(page) {
        if (page.isRedirectOriginPage() && page.isGrantedFor(req.user)) {
          ...
          return res.json(
            ApiResponse.error(sprintf('このページ名は作成できません (%s)。ページが存在します。', newPagePath)),
          )
        }
      })
      .catch(function(err) {
  ...
  function getPathMap(paths, search, replace) {
  ...
  async function checkPagesRenamable(pathMap, user) {
    let error = false
    let errors = {}
    for (let [oldPath, newPath] of Object.entries(pathMap)) {
      let e = []
      if (!Page.isCreatableName(newPath)) {
        e.push('作成できないページ名です')
      }
      const isAlreadyExists = await Page.exists({ path: newPath })
      if (isAlreadyExists) {
        const newPage = await Page.findPageByPath(newPath)
        const isUnlinkable = newPage.isRedirectOriginPage() && newPage.isGrantedFor(user)
        if (!isUnlinkable) {
          e.push('既にページが存在します')
        }
      }
      ...
      errors = Object.assign(errors, { [newPath]: e })
    }
    return [error, errors]
  }

  api.renameTree = async function(req, res) {
    const { path, new_path, create_redirect = 0 } = req.body
    const newPagePath = Page.normalizePath(new_path)

    const paths = await Page.findListByStartWith(path, req.user, {})
    const path_map = getPathMap(paths, path, newPagePath)

    const [error, errors] = await checkPagesRenamable(path_map, req.user)

    if (error) {
      const info = { errors, path_map }
      return res.json(ApiResponse.error('ページを移動できません', info))
    }

    const rename = async function(oldPath, newPath) {
      try {
        const newPageIsPortal = newPagePath.endsWith('/')
        const options = { createRedirectPage: !newPageIsPortal && create_redirect }
        const page = await Page.findPageByPath(oldPath)
        await Page.rename(page, newPath, req.user, options)
        return page
      } catch (err) {
        return Promise.reject(new Error(`Failed to update page (${oldPath}).`))
      }
    }

    const unlink = async function(page) {
      debug('Unlink page', page._id, page.path)
      try {
        const redirectPage = await Page.removePageById(page._id)
        debug('Redirect Page deleted', redirectPage.path)
      } catch (err) {
        debug('Error occured while get setting', err, err.stack)
        return Promise.reject(new Error(`Failed to delete redirect page (${page.path}).`))
      }
    }

    const unlinkIfPossible = async function(page) {
      if (page.isRedirectOriginPage() && page.isGrantedFor(req.user)) {
        await unlink(page)
      } else {
        return Promise.reject(new Error(`Failed to create this page (${page.path}). It already exists.`))
      }
    }

    try {
      const result = await Promise.all(
        Object.entries(path_map).map(async function([oldPath, newPath]) {
          if (await Page.exists({ path: newPath })) {
            const newPage = await Page.findPageByPath(newPath)
            await unlinkIfPossible(newPage)
  ...
  api.checkTreeRenamable = async function(req, res) {
    ...
    return res.json(ApiResponse.success({ path_map }))
  }
  ...
  api.unlink = function(req, res) {
    ...
    Page.findPageByIdAndGrantedUser(pageId, req.user)
      .then(function(pageData) {
        ...
        return Page.removeRedirectOriginPageByPath(pageData.path).then(() => pageData)
      })
      .then(function(data) {
        debug('Redirect Page deleted', data.path)
        var result = {}
