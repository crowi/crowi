module.exports = function(crowi, app) {
  ...
  actions.search.buildIndex = async function(req, res) {
    ...
    const newIndexName = search.createIndexName()
    try {
      await search.createIndex(newIndexName)
      debug('Index created.')
    } catch (err) {
      debug('Error', err)
      req.flash('errorMessage', 'Error while creating index.')
      return res.redirect('/admin/search')
    }

    search
      .buildIndex(newIndexName)
      .then(() => {
