...
- **中央 env 検証（boot 時に一括 validate）** — `packages/api` に散在していた `process.env` 読み取り（throw/warn/silent-fallback が場所によって不統一）を `util/env-schema.ts` の分類表 + `validateEnv()` に一本化。`Crowi` constructor が boot 時に一度だけ呼び、`PORT`/`MONGO_URI`/`REDIS_URL`/`CROWI_ENCRYPTION_KEY` の形式不正は全件まとめて即 fail-fast、`CLIENT_URL`/`CROWI_MULTI_INSTANCE`/`NODE_ENV`/`JWT_*_TTL_SECONDS`/`COLLAB_MAX_EDITORS_PER_PAGE` 等の形式不正と既知プレフィックスのタイポ疑いは起動時に 1 箇所へまとめて warning 出力（design-audit-2026-07 finding: ops-no-central-env-validation。spec: `feature-boot-env-validation.md`）
- **Plugin capability hardening（credential-vault deny-list + dependencyConfig opt-in）** — #17（plugin-capability-scoping）が書いた「`PluginContext` 経由で他プラグイン/core の secret に届かない」という doc の主張を実際に真にする。`Config`/`PersonalAccessToken`/OAuth 系（client/token/grant）/`Share`/`ShareAccess` を `modelAccess` に宣言できない deny-list を boot 検証（`assertValidModelAccess`）と呼び出し時 gate（`ctx.model()`）の両方に追加。`ctx.dependencyConfig(name)` は対象プラグインが新設の `CrowiPlugin.exposesConfigToDependents: true` を宣言していない限り throw する依存側 opt-in 制に変更（呼び出し側の `requires` 宣言だけでは読めない）。`@crowi/plugin-aws` はこのフラグを宣言し、`plugin-storage-aws-s3`/`plugin-mail-aws-ses` の既存 `dependencyConfig` 経路を維持。`@crowi/plugin-api` は破壊的変更（major、#17 の未リリース major に同乗）（2026-07-12 敵対レビュー CROWI-SEC-REVIEW-001/002。spec: `feature-plugin-capability-hardening.md`）
