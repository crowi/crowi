...
- [ ] **Phase 3 — pnpm catalog 導入** — `pnpm-workspace.yaml` に catalog 定義、横断する dev tool / 外部 peer (typescript / tsup / @types/node / jest / @ts-rest/core / zod 等) を `catalog:` 形式に。runtime dep (package ごとに版数を分けたいもの) は catalog 化しない。**advisory (simplify レビュー由来)**: 合わせて `scripts/check-workspace-protocol.mjs` (deps→`workspace:^` / devDeps→`workspace:*` を強制) + `lefthook.yml pre-commit` 連動を検討。`sort-package-json` で peer/deps/devDeps の key 並びを揃えるのも同フェーズで合流可
- [ ] **Phase 4 — `@crowi/tsconfig` パッケージ化** — `packages/tsconfig/` に `library.json` / `app-node.json` / `app-web.json` を切り、各 package が `extends: "@crowi/tsconfig/library.json"` で参照。`module: ESNext` + `moduleResolution: bundler` (library) と `module: commonjs` + `moduleResolution: node16` (app) を一元管理
