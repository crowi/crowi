...
- [ ] **Phase 7-9** — `@crowi/runner` 切り出し + dev-runner 廃止 / Dockerfile 整備 / changesets + CI publish workflow。spec は `.feature-state/specs/feature-monorepo-packages-restructure.md` 参照。plugin-api の major 昇格 (`0.1.0-dev` → `1.0.0`) は Phase 9 (changesets) と一緒に検討
