...
- [ ] **Phase 4+** — Reservation API + cache contract、AuthContext、optional plugins (katex / mermaid / plantuml / github-embed / crowi-legacy)、`addEmbedTag` / `addUrlInlineExpander` の実体実装、mention notifier hook、shiki theme 切替 (light/dark)、shiki warmup を `crowi.bootRenderer()` に組込み、`crowi-admin renderer:rebuild` (既存 revision の AST 一括再生成バッチ)、外部 plugin の core 前後挿入
