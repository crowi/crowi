...
- [x] **web image の API URL 実行時注入（汎用配布化）** — `crowi/crowi-web` を「ビルド時焼き込み」から「実行時注入」へ。同一オリジン既定（相対 `/api/v2` + Next rewrites proxy 宛先を `CROWI_API_URL` サーバ env 化 + 3 WS hook を共通 `resolve-ws-url.ts` 経由で `window.location` 導出に）、Dockerfile の `NEXT_PUBLIC_API_URL` build-arg 撤去、4 トポロジの deploy docs（operations/deployment.{en,ja}）。**クロスオリジン runtime-env escape hatch（§e: next-runtime-env / `/config.js`）は docs 化のみで実装は別タスク（YAGNI）**。api(full/slim) は先行 publish 済み、web image はこの対応後に publish（案Y）。spec: `.feature-state/specs/feature-web-image-runtime-config.md`
- [x] **web image クロスオリジン runtime-env 注入（§e フォローオン）** — `next-runtime-env`@3.3.0 の `<PublicEnvScript/>` を root layout (`src/app/layout.tsx`) の `<head>` に追加し、`NEXT_PUBLIC_API_URL` / `NEXT_PUBLIC_COLLAB_URL` をビルド焼きから実行時読み（`window.__ENV` 経由・薄い `lib/runtime-env.ts` ヘルパ）へ置換（api-client.ts / resolve-ws-url.ts）。1 個の `crowi/crowi-web` image を起動時 env だけで別オリジン api 先（HTTP + WS）に再ビルド無しで接続可能に。env 未設定時は相対 `/api/v2` + `window.location` の同一オリジン挙動を維持。deployment.mdx (B)/(C)/Summary 表を「runtime env・再ビルド不要」へ更新 + api 側 CORS（`CLIENT_URL`=web origin、exact-match）を明記。注: 3.3.0 は plain `env()` 関数を export しないため `window.__ENV` を直接読む helper を自前で用意（hook 不可の module-scope で読むため）。`output:'standalone'` + Turbopack build OK、`force-dynamic` 不要（`PublicEnvScript` の `unstable_noStore()` で layout が dynamic 化）。spec: `.feature-state/specs/feature-web-cross-origin-runtime-env.md`
...
- [ ] **MCP result body advisory (fix-mcp merge)** — `okResultWithBody`(`mcp/result.ts`)は body を `content.text` と `structuredContent.body` の両方に載せる手動オプトイン helper。将来 body を返す read tool（get-comment / attachment-content / user-profile 等）が `okResult` を使うと同じ「structuredContent 派クライアントが body を失う」バグが再発しうる。read tool が増える時に、`okResult` 側で「structured な `body` があれば text block にも mirror する」契約へ寄せて1経路化するか再検討（今は body を返す read は page/revision のみで実害なし。list/search は payload が元々 structuredContent にあるので対象外）
