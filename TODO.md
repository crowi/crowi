...
- [ ] **RFC-0004 editor advisory** — autocomplete の COLLSCAN（text index/Atlas Search 要設計）、`runPageStatusMigration` の steady-state skip、attachment fail helper / UploadIntent 共有
...
- **検索** — 全文検索 UI（`/_search` + global search + suggestion）+ admin 検索ステータス + ES 9.4.0（docker-compose 復帰）/ OpenSearch driver。index / backlink 再構築は `crowi-admin` CLI に集約
...
- **Storage driver 切替**: `crowi-admin storage copy --from <a> --to <b>`（dry-run 可）→ `crowi.config.json:storage.driver` 変更 → 再起動 → `/admin/storage` で確認。旧データは温存
