...
- [x] **並列 jest の DB 接続 flake 根治 Phase 1-2**（接続確立の bounded retry + テスト専用 mongod 分離で `connect ETIMEDOUT`/SIGSEGV を根治）。残: Phase 3 lint 強制 + collab・plugin-search-mongo 統一 / Phase 4 flake 自動検出（spec: `feature-test-parallel-db-flake-hardening.md`）
