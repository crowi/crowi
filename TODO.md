...
- [ ] **STATIC_CAPABILITIES drift guard (crowi-cli merge)** — `app-capabilities.ts` の `STATIC_CAPABILITIES` は手書きリスト(always-on subsystem)。boot 時に各 static capability がマウント済み route group に対応するか assert する test を `@crowi/api` に1本入れると、サイレント drift を CI 失敗に変えられる。consumer は WARN-only なので advisory(現状は機能停止リスクなし)。`oauth:*` タグは `GRANT_TYPES_SUPPORTED` と二重なので同期維持も同 test で守れると尚良い
