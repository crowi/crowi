...
- [x] **ページリンクの `+`/%2B 契約統一強制（Phase 1）** — `POST /pages` に draft/rename と同じ検証を追加し、fragment/query 切り落とし・malformed percent-encoding・trash 二重 decode を修正した（spec: `feature-page-link-space-paths.md`）
