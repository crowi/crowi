...
- [ ] **WS 基盤の重複抽出**（notifications/presence/collab の 3-way 重複）。`feature-live-page-content-sync` が presence チャネルに page-updated 相乗り + Redis 2 本目 subscriber (`crowi:presence:page-updated`) + `broadcastPageUpdated` を追加し、presence 側の pub/sub + broadcast パターンの duplication が 1 件増えた（viewer-list 経路のミラー）。共通基盤抽出時に page-updated 経路も一緒に畳む
