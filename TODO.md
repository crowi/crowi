...
- [ ] **admin config coercion helper の共通化**: `routes/ts-rest/admin/{app,security,auth,mail,share}.ts` で `toBoolean` / `asString` / `asPort` / `toStringValue` / `toRegistrationMode` / `toStringArray` / `asBoolean` がそれぞれ定義されている (5 ファイル)。`util/ts-rest-helpers.ts` (or new `util/admin-config.ts`) に `coerceBoolean` / `coerceString` / `coerceNumber` / `coerceStringArray` として lift。同時に `(cfg as { crowi?: ... }).crowi` の cast も `getCrowiConfigNamespace(crowi)` へ
- [ ] **`internalServerErrorResponse` const helper**: `routes/ts-rest/admin/{app,security,auth,share}.ts` 各 catch に同じ `{ status: 500, body: { error: { code: 'INTERNAL_ERROR', message: 'Internal server error' } } }` リテラルが 7+ サイト。`util/ts-rest-helpers.ts` の既存 `pageNotFoundResponse` 等に揃えて `as const` で抽出。`mail.ts` は同じ catch を持たないので、抽出時に揃えるか divergence を docs 化するかの判断が必要
- [ ] **admin 共通 i18n キー**: `admin.{app,security,auth,mail,share}.{submit,submit_pending,loading,retry,failed_to_load_title,failed_to_load_body,failed_to_save,success_saved,field_errors_summary,secret_*}` がバイトレベルで一致するキーが多数 (5 namespace で重複)。`admin.common.*` に集約 (success_saved の本文など内容が違うものは残す)
- [ ] **admin settings: 変更なし時の PUT skip**: `admin/{app,security,auth}/*-form.tsx` は formData が pristine でも保存ボタンで PUT が飛び、サーバ側で `postUpdate()` → Slack/Mailer reload まで走る。フォーム層で `formData` と server state を比較して dirty なら mutate という guard を共通化したい (mail-form / share-form は既に対処済み — その実装を他フォームに展開する案)
- [ ] **`createAdminSettingsHooks<T>` factory**: `use-admin-{app,security,auth,mail,share,users}-settings.ts` の 5+ファイルが `useQuery + useMutation + setQueryData/invalidate` の同じシェイプを繰り返している。`{ key, fetch, update, errorClass? }` を取る factory に集約できる
- [ ] **`<SecretField>` コンポーネント抽出**: `mail-settings-form.tsx` の SMTP password / AWS secret access key 用の "saved badge / clear button / clear-pending badge / undo" ブロック (~55 lines × 2) と `app-settings-form.tsx` の AWS secret ブロックが重複。`{ id, label, value, hasValue, dirty, clearRequested, onChange, onClearRequested, onUndoClear, error }` props で `components/admin/secret-field.tsx` に切り出す。mail form 単独で ~120 lines 削減可能
- [ ] **AWS schema 重複**: `schemas/admin/mail.ts` の `AwsRegionSchema` / `AwsAccessKeyIdSchema` が `schemas/admin/app.ts` と byte-for-byte 一致。`schemas/admin/_aws.ts` (or `schemas/common.ts`) に lift
- [ ] **`createPager` を util に格上げ**: `apps/crowi-api/src/routes/ts-rest/admin/users.ts:27-77` が legacy `controllers/admin.ts:22-93` の verbatim copy。今後 admin の他の list view (groups / attachments / notifications 等) にも必要なので `apps/crowi-api/src/util/admin-pager.ts` に抽出。同時に `AdminPagerSchema` も `schemas/admin/users.ts` から `schemas/admin/_pager.ts` (or `schemas/common.ts`) に移して、users の文脈に縛られないようにする
- [ ] **`User.findUsersWithPagination` のレスポンスから機密フィールドを projection で落とす**: 現状 paginate は全フィールド (password / apiToken / googleId / githubId 含む) を fetch して `toUserPublic` で client-side に strip している。Mongo→Node 間の payload を縮めるため model 側 (or admin/users handler の paginate options) で `select('-password -apiToken -googleId -githubId')` を渡す
- [ ] **`UserPublicSchema.status` を `z.nativeEnum(UserStatusEnum)` に絞る**: 現状 `z.number().optional()` で loose、フロントの formatStatus が default branch を持たざるを得ない。schema を enum 化することで union を網羅できる
- [ ] **`apps/crowi-api/src/models/user.ts` の `paginate: any` を型付け**: admin/users handler が `PaginateResult` interface を再宣言して narrow している。model 側で `paginate<T>(...): Promise<{ docs: T[]; total: number; ... }>` を提供すれば handler の hand-rolled promisify (`paginateUsers`) も `util.promisify(User.findUsersWithPagination.bind(User))` 1 行で済む
