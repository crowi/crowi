...
- [ ] **Phase 6+** — optional renderer plugins (katex / mermaid / plantuml / emoji)、`addCodeBlockRenderer` の live impl、GitHub Embed + AuthContext 本実装 (Phase 7)、mention notifier hook (Phase 8)、shiki theme 切替 (light/dark)、`crowi-admin renderer:rebuild` (既存 revision の AST 一括再生成バッチ — Phase 5.1)、`@crowi/plugin-renderer-crowi-legacy` の configSchema 分割 (Phase 5.1+)、外部 plugin の core 前後挿入
