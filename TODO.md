...
- [x] **Step 9: search driver plugin 化** — Elasticsearch (`@crowi/plugin-search-elasticsearch`) + OpenSearch (`@crowi/plugin-search-opensearch`) の 2 driver を `SearchRegistry` 経由で提供。active driver は `crowi.config.json:search.driver` で選択。詳細は Recently Completed 参照
...
- [ ] **feature-notification-realtime-invalidation merge simplify advisory (`f2aa06b5` 直後の 3-agent レビュー由来)** — WS が 3 例目になり、TODO L177 で予告していた抽出が live に:
  - **Reuse (HIGH)**: `packages/api/src/notifications/attach.ts` / `presence/attach.ts` / `collab/attach.ts` が ws-noServer upgrade filter + connection map + 6-step drain-shutdown skeleton (~60-400 行) を 3 重に持つ。`attachWsNamespace({ httpServer, crowi, path, tokenVerify, onConnection, shutdown })` を抽出して per-protocol ロジックだけ残す
  - **Reuse (HIGH)**: `util/notifications-token.ts` / `presence-token.ts` / `ws-token.ts` が `resolveSecret`(WS_TOKEN_SECRET + warn/random fallback) / factory / sign(iat/exp/jti) / verify(jwt.verify→safeParse) を 3 重に持つ。差分は issuer 文字列 + claims schema のみ。`createSignedWsTokenUtil({ issuer, ttlSeconds, payloadSchema })` に統合。**ついでに notifications-token は presence-token にある util memoize が無く、token 発行ごとに secret 再 resolve しているので統合時に memoize を入れる**
  - **Reuse (MEDIUM)**: web の `use-notifications-socket.ts` と `use-presence.ts` が reconnect backoff / close-code 分岐 / `resolve*Url`(env precedence) を重複。`lib/resolve-ws-url.ts` 抽出 (RFC-0005 advisory と同件) + WS lifecycle hook の共通化を検討
...
- [ ] **feature-watch-autosubscribe merge simplify advisory (`6b92312e` 統合後の 3-agent レビュー由来)** — reuse 指摘なし・marginal な可読性は churn 回避で見送り、以下 efficiency 2 件のみ別タスク (behavior-sensitive なので post-merge では当てず TODO 化):
  - **Efficiency (MEDIUM, behavior-sensitive)**: `packages/api/src/util/auto-watch.ts` の autoWatch が `Watcher.findByUserIdAndTargetId` → `create` の 2 ラウンドトリップ。`findOneAndUpdate` + **`$setOnInsert: { status: STATUS_WATCH }`** で 1 クエリ化できる (既存 IGNORE/WATCH を上書きしない＝IGNORE-respect 維持が必須。`auto-watch.test.ts` の IGNORE-respect ケースで担保)。comment/create/update のたびに呼ばれる経路なので効く
  - **Efficiency (MEDIUM, hot path)**: `packages/api/src/models/activity.ts` の `getNotificationTargetUsers` がページ更新 fan-out のたびに `Watcher.getWatchers` + `getIgnorers` + `User.find(active)` を発火。同一 target への UPDATE/COMMENT/LIKE が連続すると per-activity に重複クエリ。active フィルタを DB 側 (`getWatchersWithStatus` 的) に寄せる / 対象算出を activity 跨ぎで再利用する余地。fan-out hot path の DB アクセス削減
...
### feature-mongoose-major-upgrade (mongoose 6 → 8.24.0 + mongoose-paginate-v2 移行)
- [x] **deps bump** — `packages/api` / `packages/collab` の `mongoose` を `^6.13.9` → `^8.24.0` (mongodb driver 6 内包)。`packages/api` の無印 `mongoose-paginate@5.0.x` を削除し `mongoose-paginate-v2@^1.9.1` を追加 (型同梱・mongoose 7/8 対応)。挙動・API/JSON 契約は不変のバージョン追従タスク。test: api 1096 (基準 1092 + 新規 4) / collab 55、type-check api+collab 緑、lint 0 errors。
- [x] **遭遇した破壊的変更 → 修正 対応表**:
  - **callback 全廃 (mongoose 7)**: `Document#save(cb)` = `user.ts` の updatePassword/updateImage/activateInvitedUser/removeFromAdmin/makeAdmin/statusActivate/statusSuspend/statusDelete/createUserByEmailAndPassword + createUsersByInvitation 内の save → promise + 内部ブリッジ (public callback シグネチャは温存)。`Query#exec(cb)` = `user.ts` findUsers/findAdmins、`revision.ts` findLatestRevision → `.exec().then()`。`findOne(query, cb)` / `findById(id, cb)` = `user.ts` createUsersByInvitation / removeCompletelyById → promise。`Model.updateOne(q,u,opts,cb)` = `page.ts` updateRevision (planner 表に無かったが type-check で発見) → `.updateOne().then()`。`mongoose.connect(uri, opts, cb)` = `crowi/index.ts` setupDatabase → `await mongoose.connect(uri)` + try/catch (cause フォールド維持、未使用の空 opts 削除)。
  - **`Document#remove()` 削除 (mongoose 7)**: `user.ts:641` `userData.remove(cb)` → `await userData.deleteOne()`。pre('remove') hook は無いので middleware 移行不要。
  - **`findOneAndRemove` 削除 (mongoose 8)**: `bookmark.ts:163` → `findOneAndDelete` (戻り値=削除前 doc で同一)。
  - **mongoose-paginate → v2**: 戻り値 `total`→`totalDocs` / `pages`→`totalPages` (docs/limit/page は同名)。本番 consumer は `admin/users.ts` の `User.paginate` のみ。`createPager(result.totalDocs, page, result.totalPages)` でリネーム吸収 → `util/admin-pager.ts` (AdminPager wire 形) は無改修 → api-contract / web 無変更。v2 型 (`PaginateModel`/`PaginateResult`/`PaginateOptions`) は `mongoose` モジュール拡張として提供されるため `mongoose-paginate-v2` ではなく `mongoose` から import。`user.ts` の自前 PaginateResult/PaginateOptions を削除し v2 型へ。`share.ts`/`shareAccess.ts` の `paginate: any` も `PaginateModel` extends へ (本番 consumer 0)。v2 default export の plugin 型が mongoose 8 の `Schema.plugin()` generic と直接合わないため `(schema as Schema).plugin(...)` で適用。
  - **影響なし**: `Model.count()` (mongoose 由来 0)、ObjectId `new` 必須 (既に `stringToObjectId` 経由)、`id` setter 削除 (代入 0)、`strictQuery` (既に明示 `true`)。
  - **回帰確認**: collab の `page-yjs-update.ts` Buffer(payload) ↔ BSON binary round-trip は collab 55 テストで緑、driver 6 化での回帰なし。
- [x] **fast-xml-parser override は残す (結論)** — lockfile 再生成後、旧 `mongoose@6` / 無印 `mongoose-paginate` / `mongodb@4.17.2` 参照は消えたが、`mongodb@6.20.0 → (optional peer) @aws-sdk/credential-providers@3.826.0 → @aws-sdk/core@3.826.0` が `fast-xml-parser@4.4.1` を**正確に pin** しており、root の `pnpm.overrides.fast-xml-parser: 5.7.3` を外すと 4.4.1 が再混入することを実証 (override 一時除去 → `pnpm install --lockfile-only` で `fast-xml-parser@4.4.1` 復活を確認、その後復元)。よって override は維持し、commitPlan の override 除去コミットは取り下げ。

### feature-boot-progress-ui (Docker風 boot reporter + dev 統合 launcher)
- [x] **Part 1: TTY-aware boot reporter** — `packages/api/src/util/boot-reporter.ts` を新設。`init()` の 13 step + `start()` の server step を 4 レイヤー (core: encryption/database/models/redis、config: load/migrations/oauth seed、services: renderer/plugins/mailer/slack/lru、server: build/attach/listen) に集約。`crowi/index.ts` の `init()`/`start()` でレイヤー境界ごとに `reporter.beginLayer()`/`endLayer()` を呼ぶ (`step()` シグネチャは無改修、最小差分でレイヤー境界を init/start に明示)。reporter インスタンスは `crowi.bootReporter` に持たせ init→start を跨ぐ。TTY=`process.stdout.isTTY` 判定で spinner→`✓ <layer> (Nms)` 再描画 + 末尾 `🚀 API ready <url>`、非TTY=`[boot] <layer> ok (Nms)` の grep 可能 plain 1行ログ。両モードで機械可読マーカー `@@crowi:ready api <url>` を listen 完了時に 1行 emit (launcher が唯一依存する契約、人間向けバナーと別行)。`setupPlugins` の `console.log('[crowi] Loaded N plugin(s)')` は `reporter.note()` で spinner clear→出力→再描画して services レイヤーに畳む。暗号鍵未設定 / Redis 接続失敗 / CLIENT_URL 未設定の既存 `console.warn` は `crowi.bootWarn()` (= `reporter.note()` 経由) で spinner と混線せず温存。`initForCli()` は reporter を持たない (CLI は従来 quiet UX)
- [x] **DEBUG 強制を撤廃 + 失敗可視性担保** — `packages/api/package.json` の dev script から `DEBUG=crowi:*` を削除 (boot 氾濫の主因)。reporter は debug 非依存で stdout 直書きなので default で見える。DB 接続失敗 / Redis 失敗 / 致命エラーは debug ではなく既存の `console.warn`/`console.error`/`exitOnError` で必ず出るので可視性は回帰しない。さらに DEBUG が明示設定された場合は reporter を plain モードに graceful degrade (`debugEnabled` 判定) して spinner と debug 行の混線を回避
- [x] **Part 2: dev 統合 launcher** — `scripts/dev.mjs` を新設 (zero-dep ESM、既存 `scripts/*.mjs` 様式)。`node:child_process.spawn` で `pnpm exec turbo run dev --filter ...` (旧 dev script のフィルタ列そのまま) を `detached: true` (own process group) で子起動、turbo は置換せず orchestration を温存。api readiness=`@@crowi:ready api` マーカー検出 / web readiness=`:4302` HTTP probe (`node:http.get` + ECONNREFUSED→backoff リトライ、HTTP 応答が返れば status 問わず ready) / 依存ビルド=api マーカー到達時点で `^build` 完了とみなし ✓ (turbo build ログをパースしない堅い判定)。boot フェーズだけ zero-dep ANSI sticky ダッシュボード (deps/api/web 行) をオーバーレイ、両 ready で `🚀 Accepting requests` 点灯後はダッシュボード確定して通常ストリームにパススルー (常駐 sticky にしない)。SIGINT/SIGTERM は `process.kill(-child.pid, signal)` で turbo プロセスグループへ伝播しゾンビなしクリーン停止。launcher 自身が非TTY (`pnpm dev | cat`) のときはダッシュボードを描画せず子出力を素通し。root `package.json` の `dev` を `node scripts/dev.mjs` に差し替え、`dev:api`/`dev:web` 単体は turbo 直叩きのまま据え置き (壊さない)
- [x] **テスト** — `packages/api/src/util/boot-reporter.test.ts` で純関数 (formatDuration / formatReadyMarker / parseReadyMarker round-trip + turbo prefix / plain line formatters / spinnerFrame) と非TTY plain 出力 + マーカー emit + DEBUG→plain degrade + TTY バナー emit を 13 ケースでカバー。ANSI 実描画と spawn 統合は手動 smoke に委譲 (AC)。手動 smoke: `pnpm dev` (TTY 表示) / `pnpm dev | cat` (plain) / Ctrl-C (ゾンビなし) / `pnpm dev:api`・`pnpm dev:web` 単体確認

### feature-notification-realtime-invalidation (通知ポーリング廃止 + WS invalidation)
- [x] **NotificationBell の 30s polling を廃止** — `useUnreadCount` の `refetchInterval` / `refetchIntervalInBackground` と `UNREAD_COUNT_POLL_INTERVAL_MS` 定数を削除。バッジは `useNotificationsSocket` 経由のサーバ push で更新される (タブ非アクティブ時の polling もなくなった、1k 同時タブで baseline 33 RPS だった負荷が消える)。WS が落ちている時はユーザー操作に紐づく react-query の通常 refetch 経路で回収 — degrade graceful
- [x] **`GET /api/v2/notifications/token` + 60s TTL JWT** — `packages/api-contract/src/{contracts,schemas}/notification.ts` に `getNotificationsTokenRoute` / `NotificationsTokenResponseSchema` / `NotificationsTokenPayloadSchema` / `NotificationsChangedMessageSchema` を追加、`client.ts` の `lateContractApp` stub chain に登録。`packages/api/src/util/notifications-token.ts` に `createNotificationsTokenUtil()` (issuer=`crowi-notifications`、`WS_TOKEN_SECRET` を collab/presence と共有 + iss 分離で cross-replay 防止、60s TTL、`signNotificationsToken({selfUserId}) / verifyNotificationsToken(token)` の round-trip)。Hono ハンドラは `packages/api/src/hono/handlers/notification.ts` に `getNotificationsTokenRoute` を openapi 登録 (`/notifications/{id}/open` template より前に literal `/notifications/token` を登録、first-match-wins 順序)
- [x] **`/notifications/<userId>` ws noServer attach** — `packages/api/src/notifications/attach.ts` (`attachNotificationsServer(httpServer, crowi)`)。collab + presence と同じ path-filtered upgrade パターンで同 http.Server に共存。Connection lifecycle: token 検証 → URL path segment と `claims.selfUserId` が一致しなければ 4403 close、4401 = 不正トークン、4403 = path-id mismatch、1001 = SIGINT 排出。Process-local `Map<userId, Set<ws>>` で per-user 接続を保持 + lazy subscribe/unsubscribe (最初の接続で `redis.subscribe('crowi:notifications:user:<userId>')`、最後の close で `unsubscribe`)。subscriber は `crowi.redis.duplicate()` 由来の 1 本を process 全体で共有 (presence service と同パターン)。`crowi.redis === null` (REDIS_URL なし dev) の degrade mode では subscribe/publish を skip、WS は accept されるが message は届かない (spec AC)。`crowi/index.ts:start()` で `attachCollabServer` → `attachPresenceServer` → `attachNotificationsServer` の順に attach、`app.ts` SIGINT/SIGTERM teardown にも shutdown 追加
- [x] **Notification model からの publish 発火** — `models/notification.ts` の `notificationEvent.on('update', ...)` リスナーを実装、`crowi.redis?.publish('crowi:notifications:user:<userId>', JSON.stringify({type:'changed'}))` を fire-and-forget で発火。既に `upsertByActivity` / `open` は emit 済みだったので追加なし、`read` (mark-all-as-read) には emit を追加 (`Notification.updateMany` の matchedCount=0 でも emit するので「未読 0 件で押した時も他タブの "fresh" 反映」が動く)。`user` パラメータは call site によって ObjectId / UserDocument / 文字列 string が混ざるので `userIdOf` ヘルパで正規化、publish 失敗は warn のみ (handler は 200 を返す)
- [x] **Web hook `useNotificationsSocket` + (auth) 配下 1 接続** — `packages/web/src/lib/use-notifications-socket.ts`。`useNotificationsToken(enabled)` で token を react-query 管理 (expiry 30s 前 refetch、TTL 60s → cadence 30s)、`useEffect` で WS 接続 → `changed` 受信で 200ms client-debounce 後に `queryClient.invalidateQueries({queryKey: notificationKeys.all})`、(re)connect 直後にも 1 回 catch-up invalidate (切断中の取りこぼし回収)、exponential backoff 1s→15s で reconnect、4401/4403 は reconnect 停止 (token query refetch を待つ)。`(auth)/layout.tsx` で `useNotificationsSocket({enabled: isAuthenticated})` を 1 回呼ぶ — NotificationBell が常駐する全 (auth) ページで自動接続、login 画面では token API を叩かない
- [x] **テスト** — `packages/api/src/notifications/attach.test.ts` (FakeRedis 経由で valid/invalid/mismatched token + lazy subscribe/unsubscribe + per-user 隔離 + drain shutdown + degrade mode)、`packages/api/src/util/notifications-token.test.ts` (sign/verify round-trip + presence/collab issuer isolation + tamper detection + 期限切れ)、`packages/api/src/models/notification.test.ts` に publish 副作用テスト追加 (upsert/read/open 各 path が mocked `crowi.redis.publish` を呼ぶことを確認、degrade mode で no-op)、`packages/web/src/lib/use-notifications-socket.test.tsx` (FakeWebSocket + QueryClient.invalidateQueries spy で changed → invalidate / debounce 200ms / catch-up / 4401-4403 reconnect 停止 / backoff を検証)
- [x] **architectural note** — skeleton 抽出 (`attachWsNamespace({path,onConnection})` / 共通 `createSignedWsTokenUtil`) は今回スコープ外 (presence + collab + notifications で 3 例目になるので simplify advisory で見る、それまでは line-for-line のコピペで構造比較できる状態を保つ)

### feature-watch-autosubscribe (通知を watch ベースへ一本化 + 参加時オートウォッチ)
- [x] **通知 fan-out を watcher-only 化** — `models/activity.ts` の `getNotificationTargetUsers()` を `Watcher.getWatchers − getIgnorers − actionUser − 非アクティブ` に変更。暗黙集合 (`Page.getNotificationTargetUsers()` = creator + comment authors + revision authors) への依存を除去し、watch トグルを通知可否の唯一の真実にした。`Page.getNotificationTargetUsers()` は現状 dead-code だが backfill フォローアップで再利用見込みのため残置 (@deprecated note は付けず最小差分)。
- [x] **autoWatch helper** (`packages/api/src/util/auto-watch.ts`) — `autoWatchPage(Watcher, userId, pageId)`。既存行が IGNORE/WATCH なら no-op (`newlyWatching=false`)、行なしなら `Watcher.create` で WATCH を作成 (`newlyWatching=true`)。`upsertWatcher` は status を踏み潰すため使わず、`findByUserIdAndTargetId` で先に存在/status を確認する分岐。util 層の単機能モジュール (crypto.ts / collab-cap.ts 慣習) として配置、comment handler (同期) と pageEvent listener (best-effort) で共有。
- [x] **create/edit オートウォッチを単一フックポイントに集約** — `events/page.ts` の `onCreate`/`onUpdate` に `autoWatch` を追加。HTTP 保存 (`Page.createPage`/`updatePage`) と realtime collab 保存 (`collab/attach.ts` が同形状で `crowi.event('Page').emit('update', ...)` を再発火) の両経路がこの listener を通るので AC「両経路カバー」を 1 箇所で満たす。soft-delete (`Page.rename` 経由で `status='deleted'` の update が飛ぶ) は guard で skip — 削除者を watcher にしない。collab パッケージ (save-flow.ts) は触らない。
- [x] **comment オートウォッチ + `newlyWatching`** — `addComment` handler 内で `autoWatchPage` を同期 await し、`newlyWatching` をレスポンスに載せる (Comment post-save の Activity 発火は非同期なので判定に使えない)。`packages/api-contract` の `AddCommentResponseSchema` に `newlyWatching: z.boolean()` を追加、`client.ts` の stub も更新。
- [x] **getWatchStatus 単純化** — `hono/handlers/page.ts:getWatchStatusRoute` の「Watcher 行が無ければ `getNotificationTargetUsers` から導出」fallback を削除し、Watcher(WATCH) 行有無のみで `watching` を返す。web `use-watch.ts` の stale comment も更新。
- [x] **web フィードバック UI** — `use-page-comments.ts:useAddComment` が `{ comment, newlyWatching }` を返し、`newlyWatching` 時は `watchKeys.detail(pageId)` cache を `{watching:true}` に setQueryData。`page-comments.tsx` で新規 watch 時のみ inline Alert (「watch 対象にしました」+ "watch を解除" + dismiss) を表示、unwatch は既存 `useToggleWatch` 流用 (新 API 不要)。i18n `page_comments.now_watching*` を ja/en に追加。
- [x] **LIKE はオートウォッチ対象外 / backfill は別タスク** — spec 確定判断通り。liker を watcher にしない、既存ページの作成者/revision authors の WATCH 行への一括変換はやらない (今後のアクションから効く)。DB スキーマ変更なし。
- [x] **テスト** — `util/auto-watch.test.ts` (helper の create/idempotent/IGNORE-respect + pageEvent listener の create/update/soft-delete guard/IGNORE 非上書き、emit 経由で HTTP・collab 両経路カバー)、`models/activity.test.ts` に watcher-only fan-out (暗黙集合除去) テスト追加、`hono/handlers/comment.test.ts` に newlyWatching (新規=true / 2回目=false / IGNORE 尊重) 追加、`hono/handlers/page.test.ts` の getWatchStatus 既存テストを auto-watch 前提に更新 (creator は実 WATCH 行を持つ、async fire-and-forget は poll で待つ)。全 938 tests green / type-check 37 tasks / lint 0 errors / format clean。
- [ ] **advisory (後続)** — (1) 既存データ backfill タスク (`crowi-admin` バッチで既存ページの creator + revision authors を WATCH 行に変換、`Page.getNotificationTargetUsers` / `Comment.findCreatorsByPage` / `Revision.findAuthorsByPage` を再利用)。(2) backfill 完了後に `Page.getNotificationTargetUsers()` と interface decl を dead-code として削除するか判断。

### feature-plugin-search-opensearch (OpenSearch 検索ドライバ plugin + ES env 移行廃止)
- [x] **新規 `@crowi/plugin-search-opensearch`** (`b1614531`) — `packages/plugin-search-opensearch/` を新規追加 (`0.1.0-dev`、publishConfig public)。`@opensearch-project/opensearch ^3.6.0` ベースで `SearchRegistry` に `'opensearch'` driver を登録。`crowi.config.json` の `plugins[]` に追加 + `search.driver: 'opensearch'` で有効化。ES plugin (`@crowi/plugin-search-elasticsearch`) と同じ `SearchDriver` 契約 (index / remove / query / rebuild) に従い、state-ref + snapshot による hot-reload (`reconfigure` で旧 client fire-and-forget close、inflight 操作は開始時 snapshot を見続けて race-safe)、`<indexName>-current` alias の atomic swap rebuild、`default` / `kuromoji` / `sudachi` の 3 アナライザを提供。`url` は `@sensitive` annotation で AES-256-GCM 暗号化。OpenSearch SDK 3.x は全 response が `{ body: T }` でラップされ `bulk` の引数名が `operations` ではなく `body`、`indices.existsAlias` / `cat.aliases` / `cat.indices` / `indices.create` / `indices.delete` / `indices.updateAliases` も body wrap という ES 9 client との差異を driver 側で吸収。`parse-query.ts` / `query-builder.ts` / `mappings/*.json` は spec 判断で ES plugin から 1:1 コピー (共通化は 3 つめの driver 登場時に RFC で議論)。`onInstall` は実装しない (新機能なので legacy env 移行パス不要)。AWS SigV4 認証は out-of-scope (Basic Auth ベース url のみ、必要なら follow-up RFC)。analysis-kuromoji は OpenSearch では別 distribution なので operator が `bin/opensearch-plugin install analysis-kuromoji` で別途入れる旨を README に明記。test 47 件 (config-schema / driver mock / parse-query / query-builder)。
- [x] **ES plugin の env 自動移行廃止** (`c070d879`) — `@crowi/plugin-search-elasticsearch` の `onInstall` (`ELASTICSEARCH_URI` / `BONSAI_URL` 環境変数 → plugin config namespace `url` への自動移行) を削除。両 plugin の設定経路を admin UI 1 本に統一。Crowi 2.0 は alpha 開発中で本番未投入につき後方互換 fallback は負債化リスクと判断、operator は admin UI で url を設定 → `crowi-admin search rebuild` で初回 index 構築するフローに揃える。`driver.ts` の `parseUri` JSDoc から env 参照を外し (URL 形式の説明は残置)、README から legacy env 移行の 1 文を削除。
- [x] **ドキュメント / repo env references 整理** (`8e5423fc` + `72dd9c3f`) — `apps/crowi-site/content/docs/{ja,en}/operations/{search-setup,configuration}.mdx` から env 自動移行記述 (アーキ概要 / 接続情報 / トラブルシュート / 環境変数表 / Docker section) を削除し「設定は admin UI 経由のみ」に統一。repo-level の `ELASTICSEARCH_URI` 参照を `README.md` / `CLAUDE.md` / `.env.example` / `packages/api/.env.test` / `docker-compose.yml` から除去 (CLAUDE.md には admin UI 経由案内の note を追加)、`packages/api/package.json` に `@crowi/plugin-search-opensearch: workspace:*` を devDeps 追加。`.changeset/search-opensearch-plugin.md` (minor: opensearch + minor: elasticsearch) を追加。`docs/rfcs/0001-plugin-architecture.md` と TODO.md 既存履歴は historical accuracy 保持で touch せず。
...
- [x] **Phase 8 — `/_attachments?pageId=` 全ファイル一覧ページ** — 新契約 `GET /api/v2/pages/:pageId/attachments/usage` (`getAttachmentUsage`、`AttachmentUsageResponseSchema = { pagePath, latest: Attachment[], past: PastAttachmentUsage[] }`、`PastAttachmentUsage = { attachment, referencingRevisions: { revisionId, createdAt, author }[] }`)。ハンドラは `loadGrantedPage` → `Revision.find({ path: page.path }).select('_id body createdAt author').sort({ createdAt: -1 }).populate('author')` で全 revision をオンデマンド列挙 (`renderedAst` は heavy なので select から除外)、各 body を Phase 7 の `collectReferencedAttachmentIds` でスキャン。`latest` = 最新 revision 参照、`past` = 過去 revision のみ参照 + orphan (どの revision でも未参照、`referencingRevisions: []`)。キャッシュなし (`/_attachments` は明示ナビゲーションでホットパスでない)。Web は新ルート `(auth)/%5Fattachments/page.tsx` (`?pageId=` を `useSearchParams` で読む、`%5Fhistory` と同型の `'use client'` + Suspense)、`attachment-usage-view.tsx` (latest/past 2 セクション、past は各 revision へ `/<path>?revision_id=<id>` リンク)、`use-attachment-usage.ts` (react-query フック)。Phase 5 のサムネイル描画を `attachment-thumbnail.tsx` に切り出して `attachment-list` と共有、詳細 modal (Phase 6) も流用。i18n キー追加 (`page.attachments_all_*` / `page.attachments_section_*` / `page.attachments_used_in_revision` / `page.attachments_orphan`)。api test (latest/past 分割・複数 referencing revision・orphan・400/401/404) + web test (両セクション描画 + revision リンク href) を追加。
