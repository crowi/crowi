...
**判定基準は 1 つだけ**: そのエントリを外したとき、**キーの範囲 (`<pkg>@<range>` の `<range>`) を満たす版が実際に解決されるか**。キーの範囲は advisory の脆弱範囲そのものなので、この問いがそのまま「この override はまだ効いているか」と一致する。

```bash
# 各エントリについて 1 件ずつ:
# 1. そのエントリだけ一時的に削除(他のエントリは触らない)
# 2. pnpm install
# 3. lockfile の解決版を読み、キーの範囲を満たす版が現れたか semver で判定
#    — 現れた   → STILL-NEEDED (この override が押さえている。元に戻す)
#    — 現れない → REMOVABLE 候補(ただし下記の限界を読むこと)
```

**「pin 以上か」で判定しない。** major スコープの override (`brace-expansion@2` など) は自分の major しか支配しないので、pin との単純な大小比較は無関係な major を巻き込んで誤判定する。必ずキーの範囲に対する semver 判定にする。

#### 実装上の落とし穴 3 つ(いずれも実際に踏んだ)

1. **lockfile の `overrides:` セクションを解決版として拾わない。** `brace-expansion@1: 1.1.18` のような行も `<pkg>@<x>:` の形にマッチするため、`1` をバージョンとして誤読する。**`packages:` セクション以降だけ**を対象にする。
2. **`pnpm why` の出力をテキストで解析しない。** 先頭行が `<pkg>@<version>` で、階層は罫線で表現される。書式に依存した正規表現は壊れやすい。lockfile を直接読むほうが安定する。
3. **pnpm のキーには peer 情報が付く。** `@hono/node-server@2.0.11(hono@4.13.0)` のような形になるので、`(...)` を落としてから semver に渡す。

#### `REMOVABLE` の限界(機械的に削除しない)

`REMOVABLE` が示すのは「**脆弱範囲を要求する利用者が居ない**」ことだけで、「override 無しでも patched 版に解決され*続ける*」ことは保証しない。lockfile が既に patched 版を記録していると、override を外しても慣性でその版が残るためである。

したがって **同じ実行の中で自分が追加した override は、必ず `REMOVABLE` に見える**。それは「不要」ではなく「その override が版を動かした当事者」である証拠なので、消してはならない。判定を鵜呑みにせず、**その override がいつ・なぜ入ったか**と突き合わせる:

- **外してよい**: 後から別経路で解決版が上がって用済みになったもの(例: catalog の bump で `typescript-eslint` が override 無しでも patched に到達した)
- **外してはいけない**: 今まさにその override が版を押さえているもの(`pnpm update -r` で動かなかった transitive がこれに当たる)

- **1 エントリずつ確認する**(まとめて全部外して確認すると、どれが不要でどれが必須か
  切り分けられない)。
- 不要と確定したものは削除し、commit メッセージに「override `<pkg>` を削除(理由)」と
  1 行残す。削除後は **lint / type-check / 影響パッケージの test** まで通す
  (override の削除は依存ツリーを動かすため、追加と同じ検証が要る)。
- 全件確認しても不要なものが無ければ「棚卸し: 変更なし(全 override 継続要)」と
  報告するだけでよい(commit 不要)。
- この棚卸しは新規 alert が 0 件のときも独立して価値がある(`/crowi-deps` を alert
  無しで単発起動しても棚卸しだけは走らせてよい)。
- 判定が信用できないときは **1 件も削除しない**。誤った根拠で override を外すと脆弱性を
  静かに戻すので、「cruft が残る」より「誤って外す」ほうが悪い。
