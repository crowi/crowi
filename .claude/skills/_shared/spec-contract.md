...
### 内容に依存しない参照を freshness アンカーにしない

**「このファイルを触っていないこと」を確認したいだけなら、テスト計画の Test file 列にそのファイルを置かない。** validator が freshness record を作るのは Change の `symbols:` / `status: new` の path / `reuse:` / **テスト計画の Test file 列**の 4 箇所だけで、**AC 本文は走査されない**。触らないことを言いたいなら AC 本文で名指しし、参照先には置かない。

確認したいのが「変更集合の性質」(この spec の diff がそのファイルに及んでいない) であって「ファイルの中身」ではないとき、参照は grounding アンカーとして機能するだけで何の裏も取らない。しかもそのファイルが**変わる予定**なら — 典型的には所管を別 spec に譲ったファイル — その spec が着地した瞬間に stale になる。

(2026-08-28 の実例: `feature-e2e-gate-inversion` が「CI workflow を触らない」ことの確認先に `.github/workflows/ci.yml` を置いていた。所管を譲った先の `feature-e2e-drop-plantuml` が着地して ci.yml を変えた瞬間に stale になり、kickoff チェーンが停止した。)

### 1 つの規則は 1 箇所で定義する

spec は同じ規則を**設計判断・writer 表・実装マップ・処理フロー・契約・AC・テスト計画・実装順序**の複数箇所で述べる構造を持つ。**規則は 1 箇所で定義し、他の節は「§X の条件」と参照する。** 各所で言い換えて restate しない。

restate があると、改訂で一部だけが更新されて**相互排他的な指示が spec 内に並ぶ**。実装者はどちらを正とするか決められず、レビューは設計の欠陥ではなく文書内の食い違いを報告し続けることになる。とくに **AC とテスト計画は規則を書き直さず、定義側の項目を id で引く** (「表の R-3 の条件」のように)。

指摘を手で直すときも同じで、**編集前に同じ規則を述べている箇所を全て列挙してから 1 パスで直す**。1 箇所だけ直すのが、この失敗の入口である。
...
**図は任意である。ただし置くなら、散文が苦手な情報だけを運ばせる。** spec も RFC も wiki で読まれ、wiki は mermaid をそのまま描画する。

**図が要るのは、対象が「形」を持っているときだけ**である。形とは次の 2 つで、どちらも散文にすると読み手が頭の中で組み直すことになる。

- **複数の主体をまたぐ順序** — 誰が誰を呼び、何番目に何が起きるか。`sequenceDiagram` を `## 処理・データフロー` に置く。並行する 2 配送や、handler → model → DB の往復のように**主体が 2 つ以上あって順序が意味を持つ**ときに効く
- **複数の実体をまたぐ多重度** — どのコレクションがどのコレクションを何対何で指すか。`erDiagram` を使う。crowi は MongoDB なのでテーブルではなく**コレクションとフィールド**を書き、参照は `ref` の向きで描く

分岐の構造だけを見せたいなら `flowchart` を使う。**単一の関数の中の if/else を図にしない** — それは散文か、実装マップの `changes:` で足りる。

#### 図と散文の分担(重複させない)

**図は形を運び、直後の散文は図が言えないことを運ぶ。**「1 つの規則は 1 箇所で定義する」はここにも効く — 図に描いたことを下の散文で言い直すと、片方だけ直された spec ができる。

図が言えないのは、**なぜその形なのか**である: その条件が何と一致を見るのか、なぜその順序でなければ壊れるのか、失敗したらどうなるのか。図のすぐ下は、その説明のための場所として空けておく。

#### `erDiagram` は現状ではなく差分を描く

**フィールドの注釈に「このリリースで何が変わるか」を書く。** 完成後のスキーマだけを描いた図は、読み手に「で、何が新しいのか」を残す。注釈があれば図 1 枚が変更の説明になる。

```mermaid
erDiagram
  PAGES {
    ObjectId _id PK
    ObjectId revision FK "既存。参照のみ"
    string historyTracking "追加"
  }
  LIKES {
    ObjectId _id PK
    ObjectId page FK "PAGES.liker から移設"
    ObjectId user FK
  }
  PAGES ||--o{ LIKES : ""
```

注釈で書き分けるのは「追加」「削除」「既存のまま」「条件付きで不在」の 4 種で足りる。**全フィールドを列挙しない** — その spec が触るフィールドと、多重度の理解に要る PK/FK だけを描く。

#### 図を grounding アンカーにしない

**削除・改名する識別子を図に書いたまま放置しない。** 図はコードを引用しているように見えるが validator の freshness 判定は見ないので、**実装が landed した瞬間に、誰にも検出されない嘘になる**。「成功したら壊れる spec を書かない」の対象に図も含める。

消す予定のものを図に描くなら、注釈で「削除」と明示して、消えることが図の主題であるようにする。
