...
| 3 | ページ CRUD・rename・trash | partial: `onboarding.setup.ts` は API 経由 (`createPageViaApi`) で 1 ページを seed するのみ。UI 経由の CRUD/rename/trash は未保護 | `packages/api/src/hono/handlers/{page,backlink,page-portalize-twin}.ts`, `packages/api-contract/src/contracts/page.ts`, `packages/web/src/app/(auth)/[[...slug]]`, `packages/web/src/app/(auth)/trash`, `packages/web/src/lib/{use-page,use-page-mutations,use-page-list,use-page-children}.ts` |
...
| 5 | コメント | なし | `packages/api/src/hono/handlers/comment.ts`, `packages/api-contract/src/contracts/comment.ts`, `packages/web/src/lib/use-page-comments.ts`, `packages/web/src/components/page-comments/**` |
...
| 9 | 添付・アップロード | なし | `packages/api/src/hono/handlers/attachment*.ts`, `packages/api-contract/src/contracts/attachment.ts`, `packages/web/src/app/(auth)/%5Fattachments`, `packages/web/src/lib/{use-attachments,use-attachment-usage}.ts` |
